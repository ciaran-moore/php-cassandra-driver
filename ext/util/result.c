...
int
php_driver_value(const CassValue* value, const CassDataType* data_type, php8_zval *out)
{
  const char *v_string;
  size_t v_string_len;
  ...
  PHP8_ZVAL_MAYBE_MAKE(*out);

  if (cass_value_is_null(value)) {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_DEREF(out));
    return SUCCESS;
  }
  ...
  case CASS_VALUE_TYPE_VARCHAR:
    ...
    PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_DEREF(out), v_string, v_string_len);
    break;
  case CASS_VALUE_TYPE_INT:
    ...
    ZVAL_LONG(PHP8_ZVAL_MAYBE_DEREF(out), v_int_32);
    break;
  case CASS_VALUE_TYPE_COUNTER:
  case CASS_VALUE_TYPE_BIGINT:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_bigint_ce);
    numeric = PHP_DRIVER_GET_NUMERIC(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_SMALL_INT:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_smallint_ce);
    numeric = PHP_DRIVER_GET_NUMERIC(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_TINY_INT:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_tinyint_ce);
    numeric = PHP_DRIVER_GET_NUMERIC(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_TIMESTAMP:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_timestamp_ce);
    timestamp = PHP_DRIVER_GET_TIMESTAMP(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_DATE:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_date_ce);
    date = PHP_DRIVER_GET_DATE(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_TIME:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_time_ce);
    time = PHP_DRIVER_GET_TIME(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_BLOB:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_blob_ce);
    blob = PHP_DRIVER_GET_BLOB(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_VARINT:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_varint_ce);
    numeric = PHP_DRIVER_GET_NUMERIC(PHP8_ZVAL_MAYBE_DEREF(out));
    ASSERT_SUCCESS_BLOCK(cass_value_get_bytes(value, &v_bytes, &v_bytes_len),
      zval_ptr_dtor(out);
      return FAILURE;
  ...
  case CASS_VALUE_TYPE_UUID:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_uuid_ce);
    uuid = PHP_DRIVER_GET_UUID(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_TIMEUUID:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_timeuuid_ce);
    uuid = PHP_DRIVER_GET_UUID(PHP8_ZVAL_MAYBE_DEREF(out));
    ASSERT_SUCCESS_BLOCK(cass_value_get_uuid(value, &uuid->uuid),
      zval_ptr_dtor(out);
      return FAILURE;
  ...
  case CASS_VALUE_TYPE_BOOLEAN:
    ...
    if (v_boolean) {
      ZVAL_TRUE(PHP8_ZVAL_MAYBE_DEREF(out));
    } else {
      ZVAL_FALSE(PHP8_ZVAL_MAYBE_DEREF(out));
    }
    break;
  case CASS_VALUE_TYPE_INET:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_inet_ce);
    inet = PHP_DRIVER_GET_INET(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_DECIMAL:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_decimal_ce);
    numeric = PHP_DRIVER_GET_NUMERIC(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_DURATION:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_duration_ce);
    duration = PHP_DRIVER_GET_DURATION(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_DOUBLE:
    ...
    ZVAL_DOUBLE(PHP8_ZVAL_MAYBE_DEREF(out), v_double);
    break;
  case CASS_VALUE_TYPE_FLOAT:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_float_ce);
    numeric = PHP_DRIVER_GET_NUMERIC(PHP8_ZVAL_MAYBE_DEREF(out));
  ...
  case CASS_VALUE_TYPE_LIST:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_collection_ce);
    collection = PHP_DRIVER_GET_COLLECTION(PHP8_ZVAL_MAYBE_DEREF(out));
    ...
    while (cass_iterator_next(iterator)) {
      ...
      php_driver_collection_add(collection, PHP8_ZVAL_MAYBE_P(v) TSRMLS_CC);
      zval_ptr_dtor(&v);
    }
  ...
  case CASS_VALUE_TYPE_MAP:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_map_ce);
    map = PHP_DRIVER_GET_MAP(PHP8_ZVAL_MAYBE_DEREF(out));
    ...
    while (cass_iterator_next(iterator)) {
      ...
      php_driver_map_set(map, PHP8_ZVAL_MAYBE_P(k), PHP8_ZVAL_MAYBE_P(v) TSRMLS_CC);
      zval_ptr_dtor(&k);
      zval_ptr_dtor(&v);
    }
  ...
  case CASS_VALUE_TYPE_SET:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_set_ce);
    set = PHP_DRIVER_GET_SET(PHP8_ZVAL_MAYBE_DEREF(out));
    ...
    while (cass_iterator_next(iterator)) {
      ...
      php_driver_set_add(set, PHP8_ZVAL_MAYBE_P(v) TSRMLS_CC);
      zval_ptr_dtor(&v);
    }
  ...
  case CASS_VALUE_TYPE_TUPLE:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_tuple_ce);
    tuple = PHP_DRIVER_GET_TUPLE(PHP8_ZVAL_MAYBE_DEREF(out));
    ...
    while (cass_iterator_next(iterator)) {
      ...
      if (!cass_value_is_null(value)) {
        ...
        php_driver_tuple_set(tuple, index, PHP8_ZVAL_MAYBE_P(v) TSRMLS_CC);
        zval_ptr_dtor(&v);
      }
  ...
  case CASS_VALUE_TYPE_UDT:
    object_init_ex(PHP8_ZVAL_MAYBE_DEREF(out), php_driver_user_type_value_ce);
    user_type_value = PHP_DRIVER_GET_USER_TYPE_VALUE(PHP8_ZVAL_MAYBE_DEREF(out));
...
int
php_driver_get_keyspace_field(const CassKeyspaceMeta *metadata, const char *field_name, php8_zval *out)
{
...
int
php_driver_get_table_field(const CassTableMeta *metadata, const char *field_name, php8_zval *out)
{
  const CassValue *value;
...
int
php_driver_get_column_field(const CassColumnMeta *metadata, const char *field_name, php8_zval *out)
{
  const CassValue *value;
...
int
php_driver_get_result(const CassResult *result, php8_zval *out)
{
  php8_zval     rows;
  php8_zval     row;
  ...
  PHP8_ZVAL_MAYBE_MAKE(rows);
  array_init(PHP8_ZVAL_MAYBE_P(rows));
  ...
  while (cass_iterator_next(iterator)) {
    PHP8_ZVAL_MAYBE_MAKE(row);
    array_init(PHP8_ZVAL_MAYBE_P(row));
    ...
    for (i = 0; i < columns; i++) {
      ...
      PHP8_ADD_ASSOC_ZVAL_EX(PHP8_ZVAL_MAYBE_P(row),
                                column_names[i], strlen(column_names[i]) + 1,
                                PHP8_ZVAL_MAYBE_P(value));
    }

    add_next_index_zval(PHP8_ZVAL_MAYBE_P(rows),
                        PHP8_ZVAL_MAYBE_P(row));
  }
