...
php_driver_validate_object(zval *object, zval *ztype)
{
 ...
 case CASS_VALUE_TYPE_MAP:
    if (!INSTANCE_OF(php_driver_map_ce)) {
      ...
      php_driver_type *map_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(map->type));
 ...
 case CASS_VALUE_TYPE_SET:
    if (!INSTANCE_OF(php_driver_set_ce)) {
      ...
      php_driver_type *set_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(set->type));
 ...
 case CASS_VALUE_TYPE_LIST:
    if (!INSTANCE_OF(php_driver_collection_ce)) {
      ...
      php_driver_type *collection_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(collection->type));
 ...
 case CASS_VALUE_TYPE_TUPLE:
    if (!INSTANCE_OF(php_driver_tuple_ce)) {
      ...
      php_driver_type *tuple_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(tuple->type));
 ...
 case CASS_VALUE_TYPE_UDT:
    if (!INSTANCE_OF(php_driver_user_type_value_ce)) {
      ...
      php_driver_type *user_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(user_type_value->type));
...
static int
php_driver_tuple_set(CassTuple *tuple, php8_ulong index, zval *value, CassValueType type)
{
  int result = 1;
  php_driver_blob       *blob;
  php_driver_numeric    *numeric;
...
php_driver_collection_from_set(php_driver_set *set, CassCollection **collection_ptr)
{
  ...
  type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(set->type));
  value_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type->data.set.value_type));
...
php_driver_collection_from_collection(php_driver_collection *coll, CassCollection **collection_ptr)
{
  ...
  type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(coll->type));
  value_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type->data.collection.value_type));
#if CURRENT_CPP_DRIVER_VERSION > CPP_DRIVER_VERSION(2, 2, 2)
  collection = cass_collection_new_from_data_type(type->data_type,
  ...
  PHP8_ZEND_HASH_FOREACH_VAL(&coll->values, current) {
    if (!php_driver_collection_append(collection, PHP8_ZVAL_MAYBE_DEREF(current), value_type->type TSRMLS_CC)) {
      result = 0;
      break;
    }
  } PHP8_ZEND_HASH_FOREACH_END(&coll->values);
...
php_driver_collection_from_map(php_driver_map *map, CassCollection **collection_ptr)
{
  ...
  value_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type->data.map.value_type));
  key_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type->data.map.key_type));
#if CURRENT_CPP_DRIVER_VERSION > CPP_DRIVER_VERSION(2, 2, 2)
  collection = cass_collection_new_from_data_type(type->data_type,
...
php_driver_tuple_from_tuple(php_driver_tuple *tuple, CassTuple **output)
{
  ...
  PHP8_ZEND_HASH_FOREACH_NUM_KEY_VAL(&tuple->values, num_key, current) {
    php8_zval *zsub_type;
    php_driver_type *sub_type;
    if (!PHP8_ZEND_HASH_INDEX_FIND(&type->data.tuple.types, num_key, zsub_type) ||
        !php_driver_validate_object(PHP8_ZVAL_MAYBE_DEREF(current),
                                    PHP8_ZVAL_MAYBE_DEREF(zsub_type) TSRMLS_CC)) {
      result = 0;
      break;
    }
    sub_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_DEREF(zsub_type));
    if (!php_driver_tuple_set(tup, num_key,
                                 PHP8_ZVAL_MAYBE_DEREF(current), sub_type->type TSRMLS_CC)) {
      result = 0;
      break;
    }
  } PHP8_ZEND_HASH_FOREACH_END(&tuple->values);
                                             ...
                                             CassUserType **output)
{
  ...
  type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(user_type_value->type));
  ut = cass_user_type_new_from_data_type(type->data_type);

  PHP8_ZEND_HASH_FOREACH_STR_KEY_VAL(&user_type_value->values, name, current) {
    php8_zval *zsub_type;
    php_driver_type *sub_type;
    if (!PHP8_ZEND_HASH_FIND(&type->data.udt.types, name, strlen(name) + 1, zsub_type) ||
        !php_driver_validate_object(PHP8_ZVAL_MAYBE_DEREF(current),
                                       PHP8_ZVAL_MAYBE_DEREF(zsub_type) TSRMLS_CC)) {
      result = 0;
      break;
    ...
    if (!php_driver_user_type_set(ut,
                                     name,
                                     PHP8_ZVAL_MAYBE_DEREF(current),
                                     sub_type->type TSRMLS_CC)) {
      result = 0;
      break;
    }
  } PHP8_ZEND_HASH_FOREACH_END(&user_type_value->values);
