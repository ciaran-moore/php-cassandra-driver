...
php_driver_tuple_from_node(struct node_s *node) {
  ...
  for (current = node->first_child;
    ...
    php8_zval sub_type = php_driver_create_type(current TSRMLS_CC);
    php_driver_type_tuple_add(type,
                                 PHP8_ZVAL_MAYBE_P(sub_type)
                                 TSRMLS_CC);
  }
...
php_driver_user_type_from_data_type(const CassDataType *data_type)
{
  ...
  for (i = 0; i < count; ++i) {
    ...
    php_driver_type_user_type_add(type,
                                     name, name_length,
                                     PHP8_ZVAL_MAYBE_P(sub_type) TSRMLS_CC);
  }
...
#undef XX_SCALAR
  ...
  case CASS_VALUE_TYPE_LIST:
    ...
    ztype = php_driver_type_collection(PHP8_ZVAL_MAYBE_P(value_type) TSRMLS_CC);
    break;
  ...
  case CASS_VALUE_TYPE_MAP:
    ...
    ztype = php_driver_type_map(PHP8_ZVAL_MAYBE_P(key_type),
                                PHP8_ZVAL_MAYBE_P(value_type) TSRMLS_CC);
    break;

  case CASS_VALUE_TYPE_SET:
    value_type = php_driver_type_from_data_type(
      cass_data_type_sub_data_type(data_type, 0) TSRMLS_CC);
    ztype = php_driver_type_set(PHP8_ZVAL_MAYBE_P(value_type) TSRMLS_CC);
    break;
...
{
  return php_driver_type_compare(PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type1->data.collection.value_type)),
                                    PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type2->data.collection.value_type)) TSRMLS_CC);
}
...
{
  int result;
  result = php_driver_type_compare(PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type1->data.map.key_type)),
                                       PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type2->data.map.key_type)) TSRMLS_CC);
  if (result != 0) return result;
  result =  php_driver_type_compare(PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type1->data.map.value_type)),
                                       PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type2->data.map.value_type)) TSRMLS_CC);
  if (result != 0) return result;
  return 0;
}
...
{
  return php_driver_type_compare(PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type1->data.set.value_type)),
                                    PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type2->data.set.value_type)) TSRMLS_CC);
}
...
tuple_compare(php_driver_type *type1, php_driver_type *type2) {
  ...
  while (PHP8_ZEND_HASH_GET_CURRENT_DATA_EX(&type1->data.tuple.types, current1, &pos1) &&
         PHP8_ZEND_HASH_GET_CURRENT_DATA_EX(&type2->data.tuple.types, current2, &pos2)) {
    php_driver_type *sub_type1 =
        PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_DEREF(current1));
    php_driver_type *sub_type2 =
        PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_DEREF(current2));
...
user_type_compare(php_driver_type *type1, php_driver_type *type2)
{
  ...
  while (PHP8_ZEND_HASH_GET_CURRENT_KEY_EX(&type1->data.udt.types, &key1, NULL, &pos1) == HASH_KEY_IS_STRING &&
         PHP8_ZEND_HASH_GET_CURRENT_KEY_EX(&type2->data.udt.types, &key2, NULL, &pos2) == HASH_KEY_IS_STRING &&
         PHP8_ZEND_HASH_GET_CURRENT_DATA_EX(&type1->data.udt.types, current1, &pos1) &&
         PHP8_ZEND_HASH_GET_CURRENT_DATA_EX(&type2->data.udt.types, current2, &pos2)) {
    int result;
    php_driver_type *sub_type1 =
        PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_DEREF(current1));
    php_driver_type *sub_type2 =
        PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_DEREF(current2));
    result = php8_string_compare(key1, key2);
    if (result != 0) return result;
...
collection_string(php_driver_type *type, smart_str *string)
{
  ...
  php_driver_type_string(PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type->data.collection.value_type)), string TSRMLS_CC);
...
map_string(php_driver_type *type, smart_str *string)
{
  ...
  php_driver_type_string(PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type->data.map.key_type)), string TSRMLS_CC);
  smart_str_appendl(string, ", ", 2);
  php_driver_type_string(PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type->data.map.value_type)), string TSRMLS_CC);
  smart_str_appendl(string, ">", 1);
}
...
set_string(php_driver_type *type, smart_str *string)
{
  ...
  php_driver_type_string(PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(type->data.set.value_type)), string TSRMLS_CC);
  smart_str_appendl(string, ">", 1);
}
...
tuple_string(php_driver_type *type, smart_str *string) {
  ...
  PHP8_ZEND_HASH_FOREACH_VAL(&type->data.tuple.types, current) {
    php_driver_type *sub_type =
        PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_DEREF(current));
...
user_type_string(php_driver_type *type, smart_str *string)
{
  ...
  if (type->data.udt.type_name) {
    ...
    PHP8_ZEND_HASH_FOREACH_STR_KEY_VAL(&type->data.udt.types, name, current) {
      php_driver_type *sub_type =
          PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_DEREF(current));
      if (!first) smart_str_appendl(string, ", ", 2);
      first = 0;
      ...
      smart_str_appendl(string, ":", 1);
      php_driver_type_string(sub_type, string TSRMLS_CC);
    } PHP8_ZEND_HASH_FOREACH_END(&type->data.udt.types);
    smart_str_appendl(string, ">", 1);
  }
}
...
php_driver_type_scalar_new(CassValueType type)
{
  ...
  PHP8_ZVAL_MAYBE_MAKE(ztype);
  object_init_ex(PHP8_ZVAL_MAYBE_P(ztype), php_driver_type_scalar_ce);
  scalar = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(ztype));
  scalar->type = type;
...
{
  char *string;
  php8_size string_len;

  if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s", &string, &string_len) == FAILURE) {
    return;
  }

  PHP8_RETVAL_STRINGL(string, string_len);
}
...
#define XX_SCALAR(name, value) \
  if (value == type) { \
    if (PHP8_ZVAL_IS_UNDEF(PHP_DRIVER_G(TYPE_CODE(name)))) { \
      PHP_DRIVER_G(TYPE_CODE(name)) = php_driver_type_scalar_new(type TSRMLS_CC); \
    } \
    Z_ADDREF_P(PHP8_ZVAL_MAYBE_P(PHP_DRIVER_G(TYPE_CODE(name)))); \
                       ...
                       zval *value_type)
{
  ...
  PHP8_ZVAL_MAYBE_MAKE(ztype);
  object_init_ex(PHP8_ZVAL_MAYBE_P(ztype), php_driver_type_map_ce);
  map = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(ztype));

  if (!PHP8_ZVAL_IS_UNDEF_P(key_type)) {
    sub_type = PHP_DRIVER_GET_TYPE(key_type);
    cass_data_type_add_sub_type(map->data_type, sub_type->data_type);
  }

  if (!PHP8_ZVAL_IS_UNDEF_P(value_type)) {
                                        ...
                                        CassValueType value_type)
{
  ...
  sub_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(map->data.map.key_type));
  cass_data_type_add_sub_type(map->data_type, sub_type->data_type);
  sub_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(map->data.map.value_type));
  cass_data_type_add_sub_type(map->data_type, sub_type->data_type);
...
php_driver_type_set(zval *value_type)
{
  ...
  PHP8_ZVAL_MAYBE_MAKE(ztype);
  object_init_ex(PHP8_ZVAL_MAYBE_P(ztype), php_driver_type_set_ce);
  set = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(ztype));
...
php_driver_type_set_from_value_type(CassValueType type)
{
  ...
  sub_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(set->data.set.value_type));
  cass_data_type_add_sub_type(set->data_type, sub_type->data_type);
...
php_driver_type_collection(zval *value_type)
{
  ...
  PHP8_ZVAL_MAYBE_MAKE(ztype);
  object_init_ex(PHP8_ZVAL_MAYBE_P(ztype), php_driver_type_collection_ce);
  collection = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(ztype));
...
php_driver_type_collection_from_value_type(CassValueType type)
{
  ...
  sub_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(collection->data.collection.value_type));
  cass_data_type_add_sub_type(collection->data_type, sub_type->data_type);
...
php_driver_type_tuple()
{
  ...
  PHP8_ZVAL_MAYBE_MAKE(ztype);
  object_init_ex(PHP8_ZVAL_MAYBE_P(ztype), php_driver_type_tuple_ce);
...
php_driver_type_user_type()
{
  ...
  PHP8_ZVAL_MAYBE_MAKE(ztype);
  object_init_ex(PHP8_ZVAL_MAYBE_P(ztype), php_driver_type_user_type_ce);
  user_type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(ztype));
...
php_driver_type_custom(const char *name, size_t name_length)
{
  ...
  PHP8_ZVAL_MAYBE_MAKE(ztype);
  object_init_ex(PHP8_ZVAL_MAYBE_P(ztype), php_driver_type_custom_ce);
  custom = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(ztype));
...
php_driver_create_type(struct node_s *node)
{
  ...
  if (type == CASS_VALUE_TYPE_UNKNOWN) {
    php8_zval undef;
    PHP8_ZVAL_UNDEF(undef);
    return undef;
  }

  if (type == CASS_VALUE_TYPE_CUSTOM) {
    php8_zval ztype;
    smart_str class_name = PHP8_SMART_STR_INIT;
    php_driver_node_dump_to(node, &class_name);
    ztype =  php_driver_type_custom(PHP8_SMART_STR_VAL(class_name),
                                    PHP8_SMART_STR_LEN(class_name) TSRMLS_CC);
    ...
    if (node->first_child) {
      ...
      PHP8_ZVAL_UNDEF(key_type);
      PHP8_ZVAL_UNDEF(value_type);
    }
    return php_driver_type_map(PHP8_ZVAL_MAYBE_P(key_type),
                                  PHP8_ZVAL_MAYBE_P(value_type) TSRMLS_CC);
    ...
    return php_driver_type_collection(PHP8_ZVAL_MAYBE_P(value_type) TSRMLS_CC);
    ...
    return php_driver_type_set(PHP8_ZVAL_MAYBE_P(value_type) TSRMLS_CC);
