...
PHP_METHOD(FutureRows, get)
{
  ...
  if (PHP8_ZVAL_IS_UNDEF(self->rows)) {
...
static void
php_driver_future_rows_free(php8_zend_object_free *object)
{
  php_driver_future_rows *self = PHP8_ZEND_OBJECT_GET(future_rows, object);
...
php_driver_future_rows_new(zend_class_entry *ce)
{
  php_driver_future_rows *self =
      PHP8_ZEND_OBJECT_ECALLOC(future_rows, ce);
  ...
  PHP8_ZEND_OBJECT_INIT(future_rows, self, ce);
}
