...
PHP_METHOD(FutureValue, get)
{
  ...
  if (!PHP8_ZVAL_IS_UNDEF(self->value)) {
    RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->value), 1, 0);
  }
}
...
static void
php_driver_future_value_free(php8_zend_object_free *object)
{
  php_driver_future_value *self =
      PHP8_ZEND_OBJECT_GET(future_value, object);

  PHP8_ZVAL_MAYBE_DESTROY(self->value);
...
{
  php_driver_future_value *self =
      PHP8_ZEND_OBJECT_ECALLOC(future_value, ce);

  PHP8_ZVAL_UNDEF(self->value);

  PHP8_ZEND_OBJECT_INIT(future_value, self, ce);
}
