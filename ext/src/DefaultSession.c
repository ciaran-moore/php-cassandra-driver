  ...
  ZEND_HASH_FOREACH_KEY_VAL(arguments, num_key, key, current) {
    if (key) {
      rc = bind_argument_by_name(statement, key->val,
                                 PHP8_ZVAL_MAYBE_DEREF(current) TSRMLS_CC);
    } else {
      rc = bind_argument_by_index(statement, num_key, PHP8_ZVAL_MAYBE_DEREF(current) TSRMLS_CC);
    }
    if (rc == FAILURE) break;
  } PHP8_ZEND_HASH_FOREACH_END(arguments);
             ...
             cass_int64_t timestamp)
{
  ...
  php8_zval *current;
  PHP8_ZEND_HASH_FOREACH_VAL(&batch->data.batch.statements, current) {
    ...
    if (PHP8_Z_TYPE_MAYBE_P(batch_statement_entry->statement) == IS_STRING) {
      simple_statement.type = PHP_DRIVER_SIMPLE_STATEMENT;
      simple_statement.data.simple.cql = PHP8_Z_STRVAL_MAYBE_P(batch_statement_entry->statement);
      statement = &simple_statement;
    } else {
      statement = PHP_DRIVER_GET_STATEMENT(PHP8_ZVAL_MAYBE_P(batch_statement_entry->statement));
    }

    arguments = !PHP8_ZVAL_IS_UNDEF(batch_statement_entry->arguments)
                ? Z_ARRVAL_P(PHP8_ZVAL_MAYBE_P(batch_statement_entry->arguments))
                : NULL;
    ...
    cass_statement_free(stmt);
  } PHP8_ZEND_HASH_FOREACH_END(&batch->data.batch.statements);
...
PHP_METHOD(DefaultSession, execute)
{
  ...
  timeout = PHP8_ZVAL_MAYBE_P(self->default_timeout);
  ...
  if (options) {
    ...
    if (!PHP8_ZVAL_IS_UNDEF(opts->arguments))
      arguments = PHP8_Z_ARRVAL_MAYBE_P(opts->arguments);
    ...
    if (!PHP8_ZVAL_IS_UNDEF(opts->timeout))
      timeout = PHP8_ZVAL_MAYBE_P(opts->timeout);

    if (opts->serial_consistency >= 0)
      serial_consistency = opts->serial_consistency;

    if (!PHP8_ZVAL_IS_UNDEF(opts->retry_policy))
      retry_policy = (PHP_DRIVER_GET_RETRY_POLICY(PHP8_ZVAL_MAYBE_P(opts->retry_policy)))->policy;
...
PHP_METHOD(DefaultSession, prepare)
{
  ...
  if(self->persist) {
    ...
    if (PHP8_ZEND_HASH_FIND(&EG(persistent_list), hash_key, hash_key_len + 1, le) &&
  ...
  if (future == NULL) {
    ...
    if (php_driver_future_wait_timed(future, timeout TSRMLS_CC) == SUCCESS &&
      ...
      if(self->persist) {
          ...
          PHP8_ZEND_HASH_UPDATE(&EG(persistent_list), hash_key, hash_key_len + 1, &resource, sizeof(php8_zend_resource_le));
  ...
  if (self->persist) {
    if (php_driver_future_is_error(future TSRMLS_CC) == FAILURE) {
      (void) PHP8_ZEND_HASH_DEL(&EG(persistent_list), hash_key, hash_key_len + 1);
    }
    efree(hash_key);
  }
  else {
...
PHP_METHOD(DefaultSession, metrics)
{
  ...
  PHP8_ZVAL_MAYBE_MAKE(requests);
  array_init(PHP8_ZVAL_MAYBE_P(requests));
  add_assoc_long(PHP8_ZVAL_MAYBE_P(requests),
                 "min",
  ...
  add_assoc_double(PHP8_ZVAL_MAYBE_P(requests),
                   "mean_rate",
  ...
  PHP8_ZVAL_MAYBE_MAKE(stats);
  array_init(PHP8_ZVAL_MAYBE_P(stats));
  add_assoc_long(PHP8_ZVAL_MAYBE_P(stats),
                 "total_connections",
  ...
  PHP8_ZVAL_MAYBE_MAKE(errors);
  array_init(PHP8_ZVAL_MAYBE_P(errors));
  add_assoc_long(PHP8_ZVAL_MAYBE_P(errors),
                 "connection_timeouts",
  ...
  add_assoc_zval(return_value, "stats", PHP8_ZVAL_MAYBE_P(stats));
  add_assoc_zval(return_value, "requests", PHP8_ZVAL_MAYBE_P(requests));
  add_assoc_zval(return_value, "errors", PHP8_ZVAL_MAYBE_P(errors));
}
...
static void
php_driver_default_session_free(php8_zend_object_free *object)
{
  php_driver_session *self = PHP8_ZEND_OBJECT_GET(session, object);
...
{
  php_driver_session *self =
      PHP8_ZEND_OBJECT_ECALLOC(session, ce);
  ...
  PHP8_ZEND_OBJECT_INIT_EX(session, default_session, self, ce);
}
