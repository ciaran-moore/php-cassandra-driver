                              ...
                              const CassFunctionMeta *meta)
{
  ...
  object_init_ex(PHP8_ZVAL_MAYBE_P(result), php_driver_default_function_ce);

  function = PHP_DRIVER_GET_FUNCTION(PHP8_ZVAL_MAYBE_P(result));
  function->schema = php_driver_add_ref(schema);
  function->meta   = meta;

  cass_function_meta_full_name(function->meta, &full_name, &full_name_length);
  PHP8_ZVAL_MAYBE_MAKE(function->signature);
  PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_P(function->signature), full_name, full_name_length);
...
PHP_METHOD(DefaultFunction, name)
{
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->signature), 1, 0);
}
...
PHP_METHOD(DefaultFunction, simpleName)
{
  ...
  self = PHP_DRIVER_GET_FUNCTION(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->simple_name)) {
    const char *name;
    size_t name_length;
    cass_function_meta_name(self->meta, &name, &name_length);
    PHP8_ZVAL_MAYBE_MAKE(self->simple_name);
    PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_P(self->simple_name), name, name_length);
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->simple_name), 1, 0);
}
...
PHP_METHOD(DefaultFunction, arguments)
{
  ...
  if (PHP8_ZVAL_IS_UNDEF(self->arguments)) {
    ...
    PHP8_ZVAL_MAYBE_MAKE(self->arguments);
    array_init(PHP8_ZVAL_MAYBE_P(self->arguments));
    for (i = 0; i < count; ++i) {
      const char *name;
      size_t name_length;
      const CassDataType* data_type;
      if (cass_function_meta_argument(self->meta, i, &name, &name_length, &data_type) == CASS_OK) {
        php8_zval type = php_driver_type_from_data_type(data_type TSRMLS_CC);
        if (!PHP8_ZVAL_IS_UNDEF(type)) {
          PHP8_ADD_ASSOC_ZVAL_EX(PHP8_ZVAL_MAYBE_P(self->arguments),
                                    name, name_length + 1,
                                    PHP8_ZVAL_MAYBE_P(type));
        }
      }
    }
  }
...
PHP_METHOD(DefaultFunction, returnType)
{
  ...
  self = PHP_DRIVER_GET_FUNCTION(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->return_type)) {
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->return_type), 1, 0);
}
...
PHP_METHOD(DefaultFunction, language)
{
  ...
  self = PHP_DRIVER_GET_FUNCTION(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->language)) {
    const char *language;
    size_t language_length;
    cass_function_meta_language(self->meta, &language, &language_length);
    PHP8_ZVAL_MAYBE_MAKE(self->language);
    PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_P(self->language), language, language_length);
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->language), 1, 0);
}
...
PHP_METHOD(DefaultFunction, body)
{
  ...
  self = PHP_DRIVER_GET_FUNCTION(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->body)) {
    const char *body;
    size_t body_length;
    cass_function_meta_body(self->meta, &body, &body_length);
    PHP8_ZVAL_MAYBE_MAKE(self->body);
    PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_P(self->body), body, body_length);
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->body), 1, 0);
}
...
static void
php_driver_default_function_free(php8_zend_object_free *object)
{
  php_driver_function *self = PHP8_ZEND_OBJECT_GET(function, object);

  PHP8_ZVAL_MAYBE_DESTROY(self->simple_name);
  PHP8_ZVAL_MAYBE_DESTROY(self->arguments);
  PHP8_ZVAL_MAYBE_DESTROY(self->return_type);
  PHP8_ZVAL_MAYBE_DESTROY(self->signature);
  PHP8_ZVAL_MAYBE_DESTROY(self->language);
  PHP8_ZVAL_MAYBE_DESTROY(self->body);
...
{
  php_driver_function *self =
      PHP8_ZEND_OBJECT_ECALLOC(function, ce);

  PHP8_ZVAL_UNDEF(self->simple_name);
  PHP8_ZVAL_UNDEF(self->arguments);
  PHP8_ZVAL_UNDEF(self->return_type);
  PHP8_ZVAL_UNDEF(self->signature);
  PHP8_ZVAL_UNDEF(self->language);
  PHP8_ZVAL_UNDEF(self->body);

  self->schema = NULL;
  self->meta = NULL;

  PHP8_ZEND_OBJECT_INIT_EX(function, default_function, self, ce);
}
