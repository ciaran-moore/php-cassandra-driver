...
PHP_METHOD(FuturePreparedStatement, get)
{
  ...
  if (!PHP8_ZVAL_IS_UNDEF(self->prepared_statement)) {
    RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->prepared_statement), 1, 0);
  }
  ...
  PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(self->prepared_statement), return_value);
...
static void
php_driver_future_prepared_statement_free(php8_zend_object_free *object)
{
  php_driver_future_prepared_statement *self =
      PHP8_ZEND_OBJECT_GET(future_prepared_statement, object);
  ...
  PHP8_ZVAL_MAYBE_DESTROY(self->prepared_statement);
...
{
  php_driver_future_prepared_statement *self =
      PHP8_ZEND_OBJECT_ECALLOC(future_prepared_statement, ce);

  self->future = NULL;
  PHP8_ZVAL_UNDEF(self->prepared_statement);

  PHP8_ZEND_OBJECT_INIT(future_prepared_statement, self, ce);
}
