...
php_driver_user_type_value_set(php_driver_user_type_value *user_type_value,
                                  ...
                                  zval *object)
{
  PHP8_ZEND_HASH_UPDATE(&user_type_value->values,
                           name, name_length + 1,
                           object, sizeof(zval *));
...
php_driver_user_type_value_populate(php_driver_user_type_value *user_type_value, zval *array)
{
  ...
  PHP8_ZEND_HASH_FOREACH_STR_KEY_VAL(&type->data.udt.types, name, current) {
    ...
    (void) current;
    if (PHP8_ZEND_HASH_FIND(&user_type_value->values, name, name_len + 1, value)) {
      PHP8_ADD_ASSOC_ZVAL_EX(array, name, name_len + 1, PHP8_ZVAL_MAYBE_DEREF(value));
      Z_TRY_ADDREF_P(PHP8_ZVAL_MAYBE_DEREF(value));
    } else {
      PHP8_ADD_ASSOC_ZVAL_EX(array, name, name_len + 1, PHP8_ZVAL_MAYBE_P(null));
...
PHP_METHOD(UserTypeValue, __construct)
{
  ...
  PHP8_ZEND_HASH_FOREACH_STR_KEY_VAL(types, name, current) {
...
PHP_METHOD(UserTypeValue, set)
{
  ...
  if (!PHP8_ZEND_HASH_FIND(&type->data.udt.types,
                              name, name_length + 1,
                              sub_type)) {
    zend_throw_exception_ex(php_driver_invalid_argument_exception_ce, 0 TSRMLS_CC,
                            "Invalid name '%s'", name);
...
PHP_METHOD(UserTypeValue, get)
{
  ...
  if (PHP8_ZEND_HASH_FIND(&self->values,
                              name, name_length + 1,
                              value)) {
...
PHP_METHOD(UserTypeValue, count)
{
  ...
  php_driver_type *type =
      PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(self->type));
...
PHP_METHOD(UserTypeValue, current)
{
  ...
  php_driver_type *type =
      PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(self->type));
  if (PHP8_ZEND_HASH_GET_CURRENT_KEY_EX(&type->data.udt.types, &key, NULL, &self->pos) == HASH_KEY_IS_STRING) {
    php8_zval *value;
    if (PHP8_ZEND_HASH_FIND(&self->values, key->val, key->len + 1, value)) {
      RETURN_ZVAL(PHP8_ZVAL_MAYBE_DEREF(value), 1, 0);
    }
  }
}
...
{
  php8_zval values;
  php_driver_user_type_value *self = PHP8_ZEND_OBJECT_GET(user_type_value, object);
  ...
  php_driver_user_type_value_populate(self, PHP8_ZVAL_MAYBE_P(values) TSRMLS_CC);
...
php_driver_user_type_value_compare(zval *obj1, zval *obj2)
{
  ...
  type1 = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(user_type_value1->type));
  type2 = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(user_type_value2->type));
  ...
  while (PHP8_ZEND_HASH_GET_CURRENT_DATA_EX(&user_type_value1->values, current1, &pos1) &&
         PHP8_ZEND_HASH_GET_CURRENT_DATA_EX(&user_type_value2->values, current2, &pos2)) {
...
static void
php_driver_user_type_value_free(php8_zend_object_free *object)
{
  php_driver_user_type_value *self =
      PHP8_ZEND_OBJECT_GET(user_type_value, object);
...
{
  php_driver_user_type_value *self =
      PHP8_ZEND_OBJECT_ECALLOC(user_type_value, ce);
  ...
  PHP8_ZEND_OBJECT_INIT(user_type_value, self, ce);
}
