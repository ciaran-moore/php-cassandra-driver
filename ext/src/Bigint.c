...
{
  if (bigint->data.bigint.value < (cass_int64_t) PHP8_ZEND_LONG_MIN) {
    zend_throw_exception_ex(php_driver_range_exception_ce, 0 TSRMLS_CC, "Value is too small");
    return FAILURE;
  }

  if (bigint->data.bigint.value > (cass_int64_t) PHP8_ZEND_LONG_MAX) {
    zend_throw_exception_ex(php_driver_range_exception_ce, 0 TSRMLS_CC, "Value is too big");
    return FAILURE;
  }

  ZVAL_LONG(result, (php8_long) bigint->data.bigint.value);
  return SUCCESS;
}
...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_BIGINT TSRMLS_CC);
...
)
{
  ...
  PHP8_ZVAL_MAYBE_MAKE(value);
  to_string(PHP8_ZVAL_MAYBE_P(value), self TSRMLS_CC);
...
static void
php_driver_bigint_free(php8_zend_object_free *object)
{
  php_driver_numeric *self = PHP8_ZEND_OBJECT_GET(numeric, object);
...
php_driver_bigint_new(zend_class_entry *ce)
{
  ...
  PHP8_ZEND_OBJECT_INIT_EX(numeric, bigint, self, ce);
}
