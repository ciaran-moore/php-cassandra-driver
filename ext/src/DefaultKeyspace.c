...
PHP_METHOD(DefaultKeyspace, name)
{
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(value), 0, 1);
}
...
PHP_METHOD(DefaultKeyspace, table)
{
  ...
  if (PHP8_ZVAL_IS_UNDEF(ztable)) {
    return;
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(ztable), 0, 1);
}
...
PHP_METHOD(DefaultKeyspace, tables)
{
  ...
  while (cass_iterator_next(iterator)) {
    ...
    if (PHP8_ZVAL_IS_UNDEF(ztable)) {
      zval_ptr_dtor(PHP8_ZVAL_MAYBE_ADDR_OF(return_value));
      cass_iterator_free(iterator);
      return;
    } else {
      table = PHP_DRIVER_GET_TABLE(PHP8_ZVAL_MAYBE_P(ztable));

      if (PHP8_Z_TYPE_MAYBE_P(table->name) == IS_STRING) {
        PHP8_ADD_ASSOC_ZVAL_EX(return_value,
                                  PHP8_Z_STRVAL_MAYBE_P(table->name),
                                  PHP8_Z_STRLEN_MAYBE_P(table->name) + 1,
                                  PHP8_ZVAL_MAYBE_P(ztable));
      } else {
        add_next_index_zval(return_value, PHP8_ZVAL_MAYBE_P(ztable));
      }
    }
  }
...
PHP_METHOD(DefaultKeyspace, userType)
{
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(ztype), 0, 1);
}
...
int php_driver_arguments_string(php8_zval_args args,
                                   int argc,
                                   smart_str *arguments) {
  int i;

  for (i = 0; i < argc; ++i) {
    zval *argument_type = PHP8_ZVAL_ARG(args[i]);
...
PHP_METHOD(DefaultKeyspace, function)
{
  ...
  smart_str arguments = PHP8_SMART_STR_INIT;
  int argc = 0;
  ...
  meta =
      cass_keyspace_meta_function_by_name_n(self->meta,
                                            name, name_len,
                                            PHP8_SMART_STR_VAL(arguments),
                                            PHP8_SMART_STR_LEN(arguments));
  if (meta) {
    php8_zval zfunction = php_driver_create_function(self->schema, meta TSRMLS_CC);
    RETVAL_ZVAL(PHP8_ZVAL_MAYBE_P(zfunction), 1, 1);
  } else {
    RETVAL_FALSE;
  }
...
PHP_METHOD(DefaultKeyspace, functions)
{
  ...
  while (cass_iterator_next(iterator)) {
    ...
    if (!PHP8_ZVAL_IS_UNDEF(zfunction)) {
      php_driver_function *function = PHP_DRIVER_GET_FUNCTION(PHP8_ZVAL_MAYBE_P(zfunction));

      if (PHP8_Z_TYPE_MAYBE_P(function->signature) == IS_STRING) {
        PHP8_ADD_ASSOC_ZVAL_EX(return_value,
                                  PHP8_Z_STRVAL_MAYBE_P(function->signature),
                                  PHP8_Z_STRLEN_MAYBE_P(function->signature) + 1,
                                  PHP8_ZVAL_MAYBE_P(zfunction));
      } else {
        add_next_index_zval(return_value, PHP8_ZVAL_MAYBE_P(zfunction));
      }
    }
  }
                               ...
                               const CassAggregateMeta *meta)
{
  ...
  object_init_ex(PHP8_ZVAL_MAYBE_P(result), php_driver_default_aggregate_ce);

  aggregate = PHP_DRIVER_GET_AGGREGATE(PHP8_ZVAL_MAYBE_P(result));
  aggregate->schema = php_driver_add_ref(schema);
  aggregate->meta   = meta;

  cass_aggregate_meta_full_name(aggregate->meta, &full_name, &full_name_length);
  PHP8_ZVAL_MAYBE_MAKE(aggregate->signature);
  PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_P(aggregate->signature), full_name, full_name_length);
...
PHP_METHOD(DefaultKeyspace, aggregate)
{
  ...
  meta =
      cass_keyspace_meta_aggregate_by_name_n(self->meta,
                                            ...
                                            PHP8_SMART_STR_LEN(arguments));
  if (meta) {
    php8_zval zaggregate = php_driver_create_aggregate(self->schema, meta TSRMLS_CC);
    RETVAL_ZVAL(PHP8_ZVAL_MAYBE_P(zaggregate), 1, 1);
  } else {
    RETVAL_FALSE;
  }
...
PHP_METHOD(DefaultKeyspace, aggregates)
{
  ...
  while (cass_iterator_next(iterator)) {
    ...
    if (!PHP8_ZVAL_IS_UNDEF(zaggregate)) {
      php_driver_aggregate *aggregate = PHP_DRIVER_GET_AGGREGATE(PHP8_ZVAL_MAYBE_P(zaggregate));

      if (PHP8_Z_TYPE_MAYBE_P(aggregate->signature) == IS_STRING) {
        PHP8_ADD_ASSOC_ZVAL_EX(return_value,
                                  PHP8_Z_STRVAL_MAYBE_P(aggregate->signature),
                                  PHP8_Z_STRLEN_MAYBE_P(aggregate->signature) + 1,
                                  PHP8_ZVAL_MAYBE_P(zaggregate));
      } else {
        add_next_index_zval(return_value, PHP8_ZVAL_MAYBE_P(zaggregate));
      }
    }
  }
...
static void
php_driver_default_keyspace_free(php8_zend_object_free *object)
{
  php_driver_keyspace *self = PHP8_ZEND_OBJECT_GET(keyspace, object);
...
{
  php_driver_keyspace *self =
      PHP8_ZEND_OBJECT_ECALLOC(keyspace, ce);

  self->meta   = NULL;
  self->schema = NULL;

  PHP8_ZEND_OBJECT_INIT_EX(keyspace, default_keyspace, self, ce);
}
