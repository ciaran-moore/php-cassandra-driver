...
{
  PHP8_ZEND_HASH_NEXT_INDEX_INSERT(&collection->values, object, sizeof(zval *));
...
static int
php_driver_collection_get(php_driver_collection *collection, ulong index, php8_zval *zvalue)
{
  php8_zval *value;
  if (PHP8_ZEND_HASH_INDEX_FIND(&collection->values, index, value)) {
    *zvalue = *value;
    return 1;
  }
  return 0;
...
php_driver_collection_find(php_driver_collection *collection, zval *object, long *index)
{
  php8_ulong num_key;
  php8_zval *current;
  PHP8_ZEND_HASH_FOREACH_NUM_KEY_VAL(&collection->values, num_key, current) {
    zval compare;
    is_equal_function(&compare, object, PHP8_ZVAL_MAYBE_DEREF(current) TSRMLS_CC);
    if (PHP8_ZVAL_IS_TRUE_P(&compare)) {
      *index = (long) num_key;
      return 1;
    }
  } PHP8_ZEND_HASH_FOREACH_END(&collection->values);
...
{
  php8_zval *current;
  PHP8_ZEND_HASH_FOREACH_VAL(&collection->values, current) {
    if (add_next_index_zval(array, PHP8_ZVAL_MAYBE_DEREF(current)) == SUCCESS)
      Z_TRY_ADDREF_P(PHP8_ZVAL_MAYBE_DEREF(current));
    else
      break;
  } PHP8_ZEND_HASH_FOREACH_END(&collection->values);
}
...
PHP_METHOD(Collection, add)
{
  ...
  for (i = 0; i < argc; i++) {
    if (Z_TYPE_P(PHP8_ZVAL_ARG(args[i])) == IS_NULL) {
    ...
    if (!php_driver_validate_object(PHP8_ZVAL_ARG(args[i]),
                                    PHP8_ZVAL_MAYBE_P(type->data.collection.value_type) TSRMLS_CC)) {
      PHP8_MAYBE_EFREE(args);
      RETURN_FALSE;
    }
  }
  ...
  for (i = 0; i < argc; i++) {
    php_driver_collection_add(self, PHP8_ZVAL_ARG(args[i]) TSRMLS_CC);
  }
...
PHP_METHOD(Collection, current)
{
  ...
  if (PHP8_ZEND_HASH_GET_CURRENT_DATA(&collection->values, current)) {
    RETURN_ZVAL(PHP8_ZVAL_MAYBE_DEREF(current), 1, 0);
  }
}
/* }}} */
...
PHP_METHOD(Collection, key)
{
  php8_ulong num_key;
  php_driver_collection *collection = PHP_DRIVER_GET_COLLECTION(getThis());
  if (PHP8_ZEND_HASH_GET_CURRENT_KEY(&collection->values, NULL, &num_key) == HASH_KEY_IS_LONG) {
    RETURN_LONG(num_key);
  }
}
/* }}} */
...
)
{
  ...
  php_driver_collection  *self = PHP8_ZEND_OBJECT_GET(collection, object);
  ...
  php_driver_collection_populate(self, PHP8_ZVAL_MAYBE_P(values));
...
php_driver_collection_compare(zval *obj1, zval *obj2)
{
  ...
  type1 = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(collection1->type));
  type2 = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(collection2->type));
  ...
  while (PHP8_ZEND_HASH_GET_CURRENT_DATA_EX(&collection1->values, current1, &pos1) &&
         PHP8_ZEND_HASH_GET_CURRENT_DATA_EX(&collection2->values, current2, &pos2)) {
...
static void
php_driver_collection_free(php8_zend_object_free *object)
{
  php_driver_collection *self =
      PHP8_ZEND_OBJECT_GET(collection, object);
...
{
  php_driver_collection *self =
      PHP8_ZEND_OBJECT_ECALLOC(collection, ce);

  zend_hash_init(&self->values, 0, NULL, ZVAL_PTR_DTOR, 0);
  self->dirty = 1;
  PHP8_ZVAL_UNDEF(self->type);

  PHP8_ZEND_OBJECT_INIT(collection, self, ce);
}
