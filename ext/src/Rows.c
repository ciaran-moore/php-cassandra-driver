...
php_driver_rows_create(php_driver_rows *current, zval *result) {
  ...
  PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(rows->rows),
                    PHP8_ZVAL_MAYBE_P(current->next_rows));
...
PHP_METHOD(Rows, count)
{
  ...
  RETURN_LONG(zend_hash_num_elements(Z_ARRVAL_P(PHP8_ZVAL_MAYBE_P(self->rows))));
}
...
PHP_METHOD(Rows, rewind)
{
  ...
  zend_hash_internal_pointer_reset(Z_ARRVAL_P(PHP8_ZVAL_MAYBE_P(self->rows)));
}
...
PHP_METHOD(Rows, current)
{
  ...
  if (PHP8_ZEND_HASH_GET_CURRENT_DATA(PHP8_Z_ARRVAL_MAYBE_P(self->rows), entry)) {
    RETURN_ZVAL(PHP8_ZVAL_MAYBE_DEREF(entry), 1, 0);
  }
}

PHP_METHOD(Rows, key)
{
  php8_ulong num_index;
  php8_string str_index;
  php_driver_rows *self = NULL;
  ...
  if (PHP8_ZEND_HASH_GET_CURRENT_KEY(PHP8_Z_ARRVAL_MAYBE_P(self->rows),
...
PHP_METHOD(Rows, next)
{
  ...
  zend_hash_move_forward(PHP8_Z_ARRVAL_MAYBE_P(self->rows));
}
...
PHP_METHOD(Rows, valid)
{
  ...
  RETURN_BOOL(zend_hash_has_more_elements(PHP8_Z_ARRVAL_MAYBE_P(self->rows)) == SUCCESS);
}
...
PHP_METHOD(Rows, offsetExists)
{
  ...
  RETURN_BOOL(zend_hash_index_exists(PHP8_Z_ARRVAL_MAYBE_P(self->rows),
                                     (php8_ulong) Z_LVAL_P(offset)));
}
...
PHP_METHOD(Rows, offsetGet)
{
  ...
  self = PHP_DRIVER_GET_ROWS(getThis());
  if (PHP8_ZEND_HASH_INDEX_FIND(PHP8_Z_ARRVAL_MAYBE_P(self->rows), Z_LVAL_P(offset), value)) {
...
PHP_METHOD(Rows, isLastPage)
{
  ...
  if (self->result == NULL &&
      PHP8_ZVAL_IS_UNDEF(self->next_rows) &&
      PHP8_ZVAL_IS_UNDEF(self->future_next_page)) {
    RETURN_TRUE;
  }
...
PHP_METHOD(Rows, nextPage)
{
  ...
  if (!self->next_result) {
    if (!PHP8_ZVAL_IS_UNDEF(self->future_next_page)) {
      php_driver_future_rows *future_rows = NULL;

      if (!instanceof_function(PHP8_Z_OBJCE_MAYBE_P(self->future_next_page),
      ...
      future_rows = PHP_DRIVER_GET_FUTURE_ROWS(PHP8_ZVAL_MAYBE_P(self->future_next_page));
...
PHP_METHOD(Rows, nextPageAsync)
{
  ...
  if (!PHP8_ZVAL_IS_UNDEF(self->future_next_page)) {
    RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->future_next_page), 1, 0);
  }

  if (self->next_result) {
    php_driver_future_value *future_value;
    PHP8_ZVAL_MAYBE_MAKE(self->future_next_page);
    object_init_ex(PHP8_ZVAL_MAYBE_P(self->future_next_page), php_driver_future_value_ce);
    future_value = PHP_DRIVER_GET_FUTURE_VALUE(PHP8_ZVAL_MAYBE_P(self->future_next_page));
    PHP8_ZVAL_MAYBE_MAKE(future_value->value);
    php_driver_rows_create(self, PHP8_ZVAL_MAYBE_P(future_value->value) TSRMLS_CC);
    RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->future_next_page), 1, 0);
  }
  ...
  PHP8_ZVAL_MAYBE_MAKE(self->future_next_page);
  object_init_ex(PHP8_ZVAL_MAYBE_P(self->future_next_page), php_driver_future_rows_ce);
  future_rows = PHP_DRIVER_GET_FUTURE_ROWS(PHP8_ZVAL_MAYBE_P(self->future_next_page));
...
PHP_METHOD(Rows, pagingStateToken)
{
  ...
  ASSERT_SUCCESS(cass_result_paging_state_token((const CassResult *) self->result->data,
                                                &paging_state,
                                                &paging_state_size));
  PHP8_RETURN_STRINGL(paging_state, paging_state_size);
}
...
PHP_METHOD(Rows, first)
{
  ...
  zend_hash_internal_pointer_reset_ex(PHP8_Z_ARRVAL_MAYBE_P(self->rows), &pos);
  if (PHP8_ZEND_HASH_GET_CURRENT_DATA(PHP8_Z_ARRVAL_MAYBE_P(self->rows), entry)) {
    RETVAL_ZVAL(PHP8_ZVAL_MAYBE_DEREF(entry), 1, 0);
  }
}
...
static void
php_driver_rows_free(php8_zend_object_free *object)
{
  php_driver_rows *self = PHP8_ZEND_OBJECT_GET(rows, object);
  ...
  PHP8_ZVAL_MAYBE_DESTROY(self->rows);
  PHP8_ZVAL_MAYBE_DESTROY(self->next_rows);
  PHP8_ZVAL_MAYBE_DESTROY(self->future_next_page);
...
php_driver_rows_new(zend_class_entry *ce)
{
  php_driver_rows *self =
      PHP8_ZEND_OBJECT_ECALLOC(rows, ce);

  self->statement   = NULL;
  self->session     = NULL;
  self->result      = NULL;
  self->next_result = NULL;
  PHP8_ZVAL_UNDEF(self->rows);
  PHP8_ZVAL_UNDEF(self->next_rows);
  PHP8_ZVAL_UNDEF(self->future_next_page);

  PHP8_ZEND_OBJECT_INIT(rows, self, ce);
}
