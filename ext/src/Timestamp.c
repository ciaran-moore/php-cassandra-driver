...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_TIMESTAMP TSRMLS_CC);
...
)
{
  php8_zval type;
  php8_zval seconds;
  php8_zval microseconds;

  php_driver_timestamp *self = PHP8_ZEND_OBJECT_GET(timestamp, object);
  ...
  PHP8_ZVAL_MAYBE_MAKE(seconds);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(seconds), sec);
  PHP8_ZEND_HASH_UPDATE(props, "seconds", sizeof("seconds"), PHP8_ZVAL_MAYBE_P(seconds), sizeof(zval));

  PHP8_ZVAL_MAYBE_MAKE(microseconds);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(microseconds), usec);
  PHP8_ZEND_HASH_UPDATE(props, "microseconds", sizeof("microseconds"), PHP8_ZVAL_MAYBE_P(microseconds), sizeof(zval));
...
static void
php_driver_timestamp_free(php8_zend_object_free *object)
{
  php_driver_timestamp *self = PHP8_ZEND_OBJECT_GET(timestamp, object);
...
{
  php_driver_timestamp *self =
      PHP8_ZEND_OBJECT_ECALLOC(timestamp, ce);

  PHP8_ZEND_OBJECT_INIT(timestamp, self, ce);
}
