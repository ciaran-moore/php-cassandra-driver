...
to_string(zval* result, php_driver_numeric *decimal)
{
  ...
  PHP8_ZVAL_STRINGL(result, string, string_len);
  efree(string);
...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_DECIMAL TSRMLS_CC);
...
)
{
  ...
  php_driver_numeric *self = PHP8_ZEND_OBJECT_GET(numeric, object);
  ...
  php_driver_format_integer(self->data.decimal.value, &string, &string_len);
  PHP8_ZVAL_MAYBE_MAKE(PHP8_ZVAL_MAYBE_P(value));
  PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_P(value), string, string_len);
  efree(string);
  PHP8_ZEND_HASH_UPDATE(props, "value", sizeof("value"), PHP8_ZVAL_MAYBE_P(value), sizeof(zval));

  PHP8_ZVAL_MAYBE_MAKE(scale);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(scale), self->data.decimal.scale);
  PHP8_ZEND_HASH_UPDATE(props, "scale", sizeof("scale"), PHP8_ZVAL_MAYBE_P(scale), sizeof(zval));
...
static void
php_driver_decimal_free(php8_zend_object_free *object)
{
  php_driver_numeric *self = PHP8_ZEND_OBJECT_GET(numeric, object);
...
{
  php_driver_numeric *self =
      PHP8_ZEND_OBJECT_ECALLOC(numeric, ce);

  self->type = PHP_DRIVER_DECIMAL;
  self->data.decimal.scale = 0;
  mpz_init(self->data.decimal.value);

  PHP8_ZEND_OBJECT_INIT_EX(numeric, decimal, self, ce);
}
