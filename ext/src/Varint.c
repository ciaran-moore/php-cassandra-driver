...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_VARINT TSRMLS_CC);
...
static void
php_driver_varint_free(php8_zend_object_free *object)
{
  php_driver_numeric *self = PHP8_ZEND_OBJECT_GET(numeric, object);
...
php_driver_varint_new(zend_class_entry *ce)
{
  ...
  PHP8_ZEND_OBJECT_INIT_EX(numeric, varint, self, ce);
}
