...
static void to_string(zval *result, cass_int64_t value)
{
  ...
  PHP8_ZVAL_STRING(result, string);
  efree(string);
}
                     ...
                     cass_int64_t *destination )
{
  if (Z_TYPE_P(value) == IS_LONG) {
    php8_long long_value = Z_LVAL_P(value);
...
PHP_METHOD(Duration, __toString)
{
  ...
  PHP8_RETVAL_STRING(rep);
  efree(rep);
}

PHP_METHOD(Duration, type)
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_DURATION TSRMLS_CC);
...
)
{
  ...
  php_driver_duration  *self = PHP8_ZEND_OBJECT_GET(duration, object);

  php8_zval wrapped_months, wrapped_days, wrapped_nanos;
  PHP8_ZVAL_MAYBE_MAKE(wrapped_months);
  PHP8_ZVAL_MAYBE_MAKE(wrapped_days);
  PHP8_ZVAL_MAYBE_MAKE(wrapped_nanos);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(wrapped_months), self->months);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(wrapped_days), self->days);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(wrapped_nanos), self->nanos);
  PHP8_ZEND_HASH_UPDATE(props, "months", sizeof("months"), PHP8_ZVAL_MAYBE_P(wrapped_months), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "days", sizeof("days"), PHP8_ZVAL_MAYBE_P(wrapped_days), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "nanos", sizeof("nanos"), PHP8_ZVAL_MAYBE_P(wrapped_nanos), sizeof(zval));
...
static void
php_driver_duration_free(php8_zend_object_free *object)
{
  php_driver_duration *self = PHP8_ZEND_OBJECT_GET(duration, object);
...
{
  php_driver_duration *self = PHP8_ZEND_OBJECT_ECALLOC(duration, ce);
  PHP8_ZEND_OBJECT_INIT(duration, self, ce);
}
