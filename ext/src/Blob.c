...
PHP_METHOD(Blob, __toString)
{
  ...
  PHP8_RETVAL_STRINGL(hex, hex_len);
  efree(hex);
}
/* }}} */
...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_BLOB TSRMLS_CC);
...
PHP_METHOD(Blob, toBinaryString)
{
  ...
  PHP8_RETVAL_STRINGL((const char *)blob->data, blob->size);
}
/* }}} */
...
)
{
  ...
  php_driver_blob *self = PHP8_ZEND_OBJECT_GET(blob, object);
  ...
  php_driver_bytes_to_hex((const char *) self->data, self->size, &hex, &hex_len);
  PHP8_ZVAL_MAYBE_MAKE(bytes);
  PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_P(bytes), hex, hex_len);
  efree(hex);
  PHP8_ZEND_HASH_UPDATE(props, "bytes", sizeof("bytes"), PHP8_ZVAL_MAYBE_P(bytes), sizeof(zval));
...
static void
php_driver_blob_free(php8_zend_object_free *object)
{
  php_driver_blob *self = PHP8_ZEND_OBJECT_GET(blob, object);
...
{
  php_driver_blob *self =
      PHP8_ZEND_OBJECT_ECALLOC(blob, ce);

  PHP8_ZEND_OBJECT_INIT(blob, self, ce);
}
