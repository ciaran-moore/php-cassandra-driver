...
PHP_METHOD(ClusterBuilder, build)
{
  ...
  PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(cluster->default_timeout),
                    PHP8_ZVAL_MAYBE_P(self->default_timeout));
  ...
  if (self->persist) {
    ...
    if (self->persist) {
      php8_zend_resource_le *le;

      if (PHP8_ZEND_HASH_FIND(&EG(persistent_list), cluster->hash_key, cluster->hash_key_len + 1, le)) {
  ...
  if (!PHP8_ZVAL_IS_UNDEF(self->ssl_options)) {
    php_driver_ssl *options = PHP_DRIVER_GET_SSL(PHP8_ZVAL_MAYBE_P(self->ssl_options));
...
PHP_METHOD(ClusterBuilder, withDefaultTimeout)
{
  ...
  if (Z_TYPE_P(timeout) == IS_NULL) {
    PHP8_ZVAL_MAYBE_DESTROY(self->default_timeout);
    PHP8_ZVAL_UNDEF(self->default_timeout);
  } else if ((Z_TYPE_P(timeout) == IS_LONG && Z_LVAL_P(timeout) > 0) ||
             (Z_TYPE_P(timeout) == IS_DOUBLE && Z_LVAL_P(timeout) > 0)) {
    PHP8_ZVAL_MAYBE_DESTROY(self->default_timeout);
    PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(self->default_timeout), timeout);
  } else {
...
PHP_METHOD(ClusterBuilder, withContactPoints)
{
  ...
  int argc = 0, i;
  smart_str contactPoints = PHP8_SMART_STR_INIT;
  php_driver_cluster_builder *self;
...
PHP_METHOD(ClusterBuilder, withBlackListHosts)
{
  ...
  php8_zval_args args = NULL;
  int argc = 0, i;
  smart_str blacklist_hosts = PHP8_SMART_STR_INIT;
  php_driver_cluster_builder *self;
...
PHP_METHOD(ClusterBuilder, withWhiteListHosts)
{
  ...
  php8_zval_args args = NULL;
  int argc = 0, i;
  smart_str whitelist_hosts = PHP8_SMART_STR_INIT;
  php_driver_cluster_builder *self;
...
PHP_METHOD(ClusterBuilder, withBlackListDCs)
{
  ...
  php8_zval_args args = NULL;
  int argc = 0, i;
  smart_str blacklist_dcs = PHP8_SMART_STR_INIT;
  php_driver_cluster_builder *self;
...
PHP_METHOD(ClusterBuilder, withWhiteListDCs)
{
  ...
  php8_zval_args args = NULL;
  int argc = 0, i;
  smart_str whitelist_dcs = PHP8_SMART_STR_INIT;
  php_driver_cluster_builder *self;
...
PHP_METHOD(ClusterBuilder, withSSL)
{
  ...
  PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(self->ssl_options), ssl_options);
...
PHP_METHOD(ClusterBuilder, withRetryPolicy)
{
  ...
  PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(self->retry_policy), retry_policy);
...
PHP_METHOD(ClusterBuilder, withTimestampGenerator)
{
  ...
  PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(self->timestamp_gen), timestamp_gen);
...
{
  php8_zval contactPoints;
  php8_zval loadBalancingPolicy;
  php8_zval localDatacenter;
  php8_zval hostPerRemoteDatacenter;
  php8_zval useRemoteDatacenterForLocalConsistencies;
  php8_zval useTokenAwareRouting;
  php8_zval username;
  php8_zval password;
  php8_zval connectTimeout;
  php8_zval requestTimeout;
  php8_zval sslOptions;
  php8_zval defaultConsistency;
  php8_zval defaultPageSize;
  php8_zval defaultTimeout;
  php8_zval usePersistentSessions;
  php8_zval protocolVersion;
  php8_zval ioThreads;
  php8_zval coreConnectionPerHost;
  php8_zval maxConnectionsPerHost;
  php8_zval reconnectInterval;
  php8_zval latencyAwareRouting;
  php8_zval tcpNodelay;
  php8_zval tcpKeepalive;
  php8_zval retryPolicy;
  php8_zval blacklistHosts;
  php8_zval whitelistHosts;
  php8_zval blacklistDCs;
  php8_zval whitelistDCs;
  php8_zval timestampGen;
  php8_zval schemaMetadata;
  php8_zval hostnameResolution;
  php8_zval randomizedContactPoints;
  php8_zval connectionHeartbeatInterval;
  ...
  PHP8_ZVAL_MAYBE_MAKE(contactPoints);
  PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(contactPoints), self->contact_points);

  PHP8_ZVAL_MAYBE_MAKE(loadBalancingPolicy);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(loadBalancingPolicy), self->load_balancing_policy);

  PHP8_ZVAL_MAYBE_MAKE(localDatacenter);
  PHP8_ZVAL_MAYBE_MAKE(hostPerRemoteDatacenter);
  PHP8_ZVAL_MAYBE_MAKE(useRemoteDatacenterForLocalConsistencies);
  if (self->load_balancing_policy == LOAD_BALANCING_DC_AWARE_ROUND_ROBIN) {
    PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(localDatacenter), self->local_dc);
    ZVAL_LONG(PHP8_ZVAL_MAYBE_P(hostPerRemoteDatacenter), self->used_hosts_per_remote_dc);
    ZVAL_BOOL(PHP8_ZVAL_MAYBE_P(useRemoteDatacenterForLocalConsistencies), self->allow_remote_dcs_for_local_cl);
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(localDatacenter));
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(hostPerRemoteDatacenter));
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(useRemoteDatacenterForLocalConsistencies));
  }

  PHP8_ZVAL_MAYBE_MAKE(useTokenAwareRouting);
  ZVAL_BOOL(PHP8_ZVAL_MAYBE_P(useTokenAwareRouting), self->use_token_aware_routing);

  PHP8_ZVAL_MAYBE_MAKE(username);
  PHP8_ZVAL_MAYBE_MAKE(password);
  if (self->username) {
    PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(username), self->username);
    PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(password), self->password);
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(username));
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(password));
  }

  PHP8_ZVAL_MAYBE_MAKE(connectTimeout);
  ZVAL_DOUBLE(PHP8_ZVAL_MAYBE_P(connectTimeout), (double) self->connect_timeout / 1000);
  PHP8_ZVAL_MAYBE_MAKE(requestTimeout);
  ZVAL_DOUBLE(PHP8_ZVAL_MAYBE_P(requestTimeout), (double) self->request_timeout / 1000);

  PHP8_ZVAL_MAYBE_MAKE(sslOptions);
  if (!PHP8_ZVAL_IS_UNDEF(self->ssl_options)) {
    PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(sslOptions), PHP8_ZVAL_MAYBE_P(self->ssl_options));
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(sslOptions));
  }

  PHP8_ZVAL_MAYBE_MAKE(defaultConsistency);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(defaultConsistency), self->default_consistency);
  PHP8_ZVAL_MAYBE_MAKE(defaultPageSize);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(defaultPageSize), self->default_page_size);
  PHP8_ZVAL_MAYBE_MAKE(defaultTimeout);
  if (!PHP8_ZVAL_IS_UNDEF(self->default_timeout)) {
    ZVAL_LONG(PHP8_ZVAL_MAYBE_P(defaultTimeout), PHP8_Z_LVAL_MAYBE_P(self->default_timeout));
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(defaultTimeout));
  }

  PHP8_ZVAL_MAYBE_MAKE(usePersistentSessions);
  ZVAL_BOOL(PHP8_ZVAL_MAYBE_P(usePersistentSessions), self->persist);

  PHP8_ZVAL_MAYBE_MAKE(protocolVersion);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(protocolVersion), self->protocol_version);

  PHP8_ZVAL_MAYBE_MAKE(ioThreads);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(ioThreads), self->io_threads);

  PHP8_ZVAL_MAYBE_MAKE(coreConnectionPerHost);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(coreConnectionPerHost), self->core_connections_per_host);

  PHP8_ZVAL_MAYBE_MAKE(maxConnectionsPerHost);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(maxConnectionsPerHost), self->max_connections_per_host);

  PHP8_ZVAL_MAYBE_MAKE(reconnectInterval);
  ZVAL_DOUBLE(PHP8_ZVAL_MAYBE_P(reconnectInterval), (double) self->reconnect_interval / 1000);

  PHP8_ZVAL_MAYBE_MAKE(latencyAwareRouting);
  ZVAL_BOOL(PHP8_ZVAL_MAYBE_P(latencyAwareRouting), self->enable_latency_aware_routing);

  PHP8_ZVAL_MAYBE_MAKE(tcpNodelay);
  ZVAL_BOOL(PHP8_ZVAL_MAYBE_P(tcpNodelay), self->enable_tcp_nodelay);

  PHP8_ZVAL_MAYBE_MAKE(tcpKeepalive);
  if (self->enable_tcp_keepalive) {
    ZVAL_DOUBLE(PHP8_ZVAL_MAYBE_P(tcpKeepalive), (double) self->tcp_keepalive_delay / 1000);
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(tcpKeepalive));
  }

  PHP8_ZVAL_MAYBE_MAKE(retryPolicy);
  if (!PHP8_ZVAL_IS_UNDEF(self->retry_policy)) {
    PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(retryPolicy), PHP8_ZVAL_MAYBE_P(self->retry_policy));
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(retryPolicy));
  }

  PHP8_ZVAL_MAYBE_MAKE(blacklistHosts);
  if (self->blacklist_hosts) {
    PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(blacklistHosts), self->blacklist_hosts);
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(blacklistHosts));
  }

  PHP8_ZVAL_MAYBE_MAKE(whitelistHosts);
  if (self->whitelist_hosts) {
    PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(whitelistHosts), self->whitelist_hosts);
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(whitelistHosts));
  }

  PHP8_ZVAL_MAYBE_MAKE(blacklistDCs);
  if (self->blacklist_dcs) {
    PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(blacklistDCs), self->blacklist_dcs);
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(blacklistDCs));
  }

  PHP8_ZVAL_MAYBE_MAKE(whitelistDCs);
  if (self->whitelist_dcs) {
    PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(whitelistDCs), self->whitelist_dcs);
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(whitelistDCs));
  }

  PHP8_ZVAL_MAYBE_MAKE(timestampGen);
  if (!PHP8_ZVAL_IS_UNDEF(self->timestamp_gen)) {
    PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(timestampGen), PHP8_ZVAL_MAYBE_P(self->timestamp_gen));
  } else {
    ZVAL_NULL(PHP8_ZVAL_MAYBE_P(timestampGen));
  }

  PHP8_ZVAL_MAYBE_MAKE(schemaMetadata);
  ZVAL_BOOL(PHP8_ZVAL_MAYBE_P(schemaMetadata), self->enable_schema);

  PHP8_ZVAL_MAYBE_MAKE(hostnameResolution);
  ZVAL_BOOL(PHP8_ZVAL_MAYBE_P(hostnameResolution), self->enable_hostname_resolution);

  PHP8_ZVAL_MAYBE_MAKE(randomizedContactPoints);
  ZVAL_BOOL(PHP8_ZVAL_MAYBE_P(randomizedContactPoints), self->enable_randomized_contact_points);

  PHP8_ZVAL_MAYBE_MAKE(connectionHeartbeatInterval);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(connectionHeartbeatInterval), self->connection_heartbeat_interval);

  PHP8_ZEND_HASH_UPDATE(props, "contactPoints", sizeof("contactPoints"),
                           PHP8_ZVAL_MAYBE_P(contactPoints), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "loadBalancingPolicy", sizeof("loadBalancingPolicy"),
                           PHP8_ZVAL_MAYBE_P(loadBalancingPolicy), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "localDatacenter", sizeof("localDatacenter"),
                           PHP8_ZVAL_MAYBE_P(localDatacenter), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "hostPerRemoteDatacenter", sizeof("hostPerRemoteDatacenter"),
                           PHP8_ZVAL_MAYBE_P(hostPerRemoteDatacenter), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "useRemoteDatacenterForLocalConsistencies", sizeof("useRemoteDatacenterForLocalConsistencies"),
                           PHP8_ZVAL_MAYBE_P(useRemoteDatacenterForLocalConsistencies), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "useTokenAwareRouting", sizeof("useTokenAwareRouting"),
                           PHP8_ZVAL_MAYBE_P(useTokenAwareRouting), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "username", sizeof("username"),
                           PHP8_ZVAL_MAYBE_P(username), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "password", sizeof("password"),
                           PHP8_ZVAL_MAYBE_P(password), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "connectTimeout", sizeof("connectTimeout"),
                           PHP8_ZVAL_MAYBE_P(connectTimeout), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "requestTimeout", sizeof("requestTimeout"),
                           PHP8_ZVAL_MAYBE_P(requestTimeout), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "sslOptions", sizeof("sslOptions"),
                           PHP8_ZVAL_MAYBE_P(sslOptions), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "defaultConsistency", sizeof("defaultConsistency"),
                           PHP8_ZVAL_MAYBE_P(defaultConsistency), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "defaultPageSize", sizeof("defaultPageSize"),
                           PHP8_ZVAL_MAYBE_P(defaultPageSize), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "defaultTimeout", sizeof("defaultTimeout"),
                           PHP8_ZVAL_MAYBE_P(defaultTimeout), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "usePersistentSessions", sizeof("usePersistentSessions"),
                           PHP8_ZVAL_MAYBE_P(usePersistentSessions), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "protocolVersion", sizeof("protocolVersion"),
                           PHP8_ZVAL_MAYBE_P(protocolVersion), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "ioThreads", sizeof("ioThreads"),
                           PHP8_ZVAL_MAYBE_P(ioThreads), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "coreConnectionPerHost", sizeof("coreConnectionPerHost"),
                           PHP8_ZVAL_MAYBE_P(coreConnectionPerHost), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "maxConnectionsPerHost", sizeof("maxConnectionsPerHost"),
                           PHP8_ZVAL_MAYBE_P(maxConnectionsPerHost), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "reconnectInterval", sizeof("reconnectInterval"),
                           PHP8_ZVAL_MAYBE_P(reconnectInterval), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "latencyAwareRouting", sizeof("latencyAwareRouting"),
                           PHP8_ZVAL_MAYBE_P(latencyAwareRouting), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "tcpNodelay", sizeof("tcpNodelay"),
                           PHP8_ZVAL_MAYBE_P(tcpNodelay), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "tcpKeepalive", sizeof("tcpKeepalive"),
                           PHP8_ZVAL_MAYBE_P(tcpKeepalive), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "retryPolicy", sizeof("retryPolicy"),
                           PHP8_ZVAL_MAYBE_P(retryPolicy), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "timestampGenerator", sizeof("timestampGenerator"),
                           PHP8_ZVAL_MAYBE_P(timestampGen), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "schemaMetadata", sizeof("schemaMetadata"),
                           PHP8_ZVAL_MAYBE_P(schemaMetadata), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "blacklist_hosts", sizeof("blacklist_hosts"),
                           PHP8_ZVAL_MAYBE_P(blacklistHosts), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "whitelist_hosts", sizeof("whitelist_hosts"),
                           PHP8_ZVAL_MAYBE_P(whitelistHosts), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "blacklist_dcs", sizeof("blacklist_dcs"),
                           PHP8_ZVAL_MAYBE_P(blacklistDCs), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "whitelist_dcs", sizeof("whitelist_dcs"),
                           PHP8_ZVAL_MAYBE_P(whitelistDCs), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "hostnameResolution", sizeof("hostnameResolution"),
                           PHP8_ZVAL_MAYBE_P(hostnameResolution), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "randomizedContactPoints", sizeof("randomizedContactPoints"),
                           PHP8_ZVAL_MAYBE_P(randomizedContactPoints), sizeof(zval));
  PHP8_ZEND_HASH_UPDATE(props, "connectionHeartbeatInterval", sizeof("connectionHeartbeatInterval"),
                           PHP8_ZVAL_MAYBE_P(connectionHeartbeatInterval), sizeof(zval));
...
static void
php_driver_cluster_builder_free(php8_zend_object_free *object)
{
  php_driver_cluster_builder *self =
      PHP8_ZEND_OBJECT_GET(cluster_builder, object);
  ...
  PHP8_ZVAL_MAYBE_DESTROY(self->ssl_options);
  PHP8_ZVAL_MAYBE_DESTROY(self->default_timeout);
  PHP8_ZVAL_MAYBE_DESTROY(self->retry_policy);
  PHP8_ZVAL_MAYBE_DESTROY(self->timestamp_gen);
...
{
  php_driver_cluster_builder *self =
      PHP8_ZEND_OBJECT_ECALLOC(cluster_builder, ce);
  ...
  PHP8_ZVAL_UNDEF(self->ssl_options);
  PHP8_ZVAL_UNDEF(self->default_timeout);
  PHP8_ZVAL_UNDEF(self->retry_policy);
  PHP8_ZVAL_UNDEF(self->timestamp_gen);

  PHP8_ZEND_OBJECT_INIT(cluster_builder, self, ce);
}
