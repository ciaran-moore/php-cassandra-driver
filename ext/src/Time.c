...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_TIME TSRMLS_CC);
...
PHP_METHOD(Time, fromDateTime)
{
  ...
  if (!PHP8_ZVAL_IS_UNDEF(retval) &&
    ...
    object_init_ex(return_value, php_driver_time_ce);
    self = PHP_DRIVER_GET_TIME(return_value);
    self->time = cass_time_from_epoch(PHP8_Z_LVAL_MAYBE_P(retval));
...
{
  php8_zval type;
  php8_zval nanoseconds;

  php_driver_time *self = PHP8_ZEND_OBJECT_GET(time, object);
  ...
  PHP8_ZVAL_MAYBE_MAKE(nanoseconds);
  to_string(PHP8_ZVAL_MAYBE_P(nanoseconds), self TSRMLS_CC);
  PHP8_ZEND_HASH_UPDATE(props, "nanoseconds", sizeof("nanoseconds"), PHP8_ZVAL_MAYBE_P(nanoseconds), sizeof(zval));
...
static void
php_driver_time_free(php8_zend_object_free *object)
{
  php_driver_time *self = PHP8_ZEND_OBJECT_GET(time, object);
...
{
  php_driver_time *self =
      PHP8_ZEND_OBJECT_ECALLOC(time, ce);

  self->time = 0;

  PHP8_ZEND_OBJECT_INIT(time, self, ce);
}
