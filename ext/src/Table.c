...
php_driver_table_build_options(CassIterator* iterator) {
  ...
  PHP8_ZVAL_MAYBE_MAKE(zoptions);
  array_init(PHP8_ZVAL_MAYBE_P(zoptions));
  while (cass_iterator_next(iterator)) {
    ...
    if (cass_iterator_get_meta_field_name(iterator, &name, &name_length) == CASS_OK) {
      ...
      if (value) {
        ...
        if (data_type) {
          php8_zval zvalue;
          PHP8_ZVAL_UNDEF(zvalue);
          if (php_driver_value(value,
                                  data_type,
                                  &zvalue TSRMLS_CC) == SUCCESS) {
            PHP8_ADD_ASSOC_ZVAL_EX(PHP8_ZVAL_MAYBE_P(zoptions),
                                      name, name_length + 1,
                                      PHP8_ZVAL_MAYBE_P(zvalue));
          }
        }
      }
    }
