...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_DATE TSRMLS_CC);
...
PHP_METHOD(Date, toDateTime)
{
  ...
  RETVAL_ZVAL(PHP8_ZVAL_MAYBE_P(datetime), 0, 1);
}
/* }}} */
...
PHP_METHOD(Date, fromDateTime)
{
  ...
  if (!PHP8_ZVAL_IS_UNDEF(retval) &&
      Z_TYPE_P(PHP8_ZVAL_MAYBE_P(retval)) == IS_LONG) {
    object_init_ex(return_value, php_driver_date_ce);
    self = PHP_DRIVER_GET_DATE(return_value);
    self->date = cass_date_from_epoch(PHP8_Z_LVAL_MAYBE_P(retval));
...
)
{
  ...
  php_driver_date *self = PHP8_ZEND_OBJECT_GET(date, object);
  ...
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(seconds), cass_date_time_to_epoch(self->date, 0));
...
static void
php_driver_date_free(php8_zend_object_free *object)
{
  php_driver_date *self = PHP8_ZEND_OBJECT_GET(date, object);
...
{
  php_driver_date *self =
      PHP8_ZEND_OBJECT_ECALLOC(date, ce);

  self->date = 0;

  PHP8_ZEND_OBJECT_INIT(date, self, ce);
}
