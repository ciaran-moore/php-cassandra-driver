...
{
  ZVAL_LONG(result, (php8_long) tinyint->data.tinyint.value);
  return SUCCESS;
}
...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_TINY_INT TSRMLS_CC);
...
static void
php_driver_tinyint_free(php8_zend_object_free *object)
{
  php_driver_numeric *self = PHP8_ZEND_OBJECT_GET(numeric, object);
...
php_driver_tinyint_new(zend_class_entry *ce)
{
  ...
  PHP8_ZEND_OBJECT_INIT_EX(numeric, tinyint, self, ce);
}
