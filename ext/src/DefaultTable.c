...
populate_partition_key(php_driver_table *table, zval *result)
{
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (column) {
      php8_zval zcolumn = php_driver_create_column(table->schema, column TSRMLS_CC);
      if (!PHP8_ZVAL_IS_UNDEF(zcolumn)) {
        add_next_index_zval(result, PHP8_ZVAL_MAYBE_P(zcolumn));
      }
    }
  }
}
                           ...
                           const CassTableMeta *meta)
{
  ...
  object_init_ex(PHP8_ZVAL_MAYBE_P(result), php_driver_default_table_ce);

  table = PHP_DRIVER_GET_TABLE(PHP8_ZVAL_MAYBE_P(result));
  table->schema = php_driver_add_ref(schema);
  table->meta   = meta;

  cass_table_meta_name(meta, &name, &name_length);
  PHP8_ZVAL_MAYBE_MAKE(table->name);
  PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_P(table->name), name, name_length);
...
php_driver_table_get_option(php_driver_table *table,
                               ...
                               zval *result) {
  zval *zvalue;
  if (PHP8_ZVAL_IS_UNDEF(table->options)) {
    php_driver_default_table_build_options(table TSRMLS_CC);
  }

  if (!PHP8_ZEND_HASH_FIND(PHP8_Z_ARRVAL_MAYBE_P(table->options),
                         name, strlen(name) + 1,
                         zvalue)) {
    ZVAL_FALSE(result);
    return;
...
PHP_METHOD(DefaultTable, column)
{
  ...
  if (PHP8_ZVAL_IS_UNDEF(column)) {
    return;
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(column), 0, 1);
}
...
PHP_METHOD(DefaultTable, columns)
{
  ...
  while (cass_iterator_next(iterator)) {
    ...
    if (!PHP8_ZVAL_IS_UNDEF(zcolumn)) {
      column = PHP_DRIVER_GET_COLUMN(PHP8_ZVAL_MAYBE_P(zcolumn));

      if (PHP8_Z_TYPE_MAYBE_P(column->name) == IS_STRING) {
        PHP8_ADD_ASSOC_ZVAL_EX(return_value,
                                  PHP8_Z_STRVAL_MAYBE_P(column->name),
                                  PHP8_Z_STRLEN_MAYBE_P(column->name) + 1,
                                  PHP8_ZVAL_MAYBE_P(zcolumn));
      } else {
        add_next_index_zval(return_value, PHP8_ZVAL_MAYBE_P(zcolumn));
      }
    }
  }
...
PHP_METHOD(DefaultTable, partitionKey)
{
  ...
  self = PHP_DRIVER_GET_TABLE(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->partition_key)) {
    PHP8_ZVAL_MAYBE_MAKE(self->partition_key);
    array_init(PHP8_ZVAL_MAYBE_P(self->partition_key));
    populate_partition_key(self, PHP8_ZVAL_MAYBE_P(self->partition_key) TSRMLS_CC);
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->partition_key), 1, 0);
}
...
PHP_METHOD(DefaultTable, primaryKey)
{
  ...
  self = PHP_DRIVER_GET_TABLE(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->primary_key)) {
    PHP8_ZVAL_MAYBE_MAKE(self->primary_key);
    array_init(PHP8_ZVAL_MAYBE_P(self->primary_key));
    populate_partition_key(self, PHP8_ZVAL_MAYBE_P(self->primary_key) TSRMLS_CC);
    populate_clustering_key(self, PHP8_ZVAL_MAYBE_P(self->primary_key) TSRMLS_CC);
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->primary_key), 1, 0);
}
...
PHP_METHOD(DefaultTable, clusteringKey)
{
  ...
  self = PHP_DRIVER_GET_TABLE(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->clustering_key)) {
    PHP8_ZVAL_MAYBE_MAKE(self->clustering_key);
    array_init(PHP8_ZVAL_MAYBE_P(self->clustering_key));
    populate_clustering_key(self, PHP8_ZVAL_MAYBE_P(self->clustering_key) TSRMLS_CC);
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->clustering_key), 1, 0);
}
...
PHP_METHOD(DefaultTable, clusteringOrder)
{
  ...
  self = PHP_DRIVER_GET_TABLE(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->clustering_order)) {
    size_t i, count = cass_table_meta_clustering_key_count(self->meta);
    PHP8_ZVAL_MAYBE_MAKE(self->clustering_order);
    array_init(PHP8_ZVAL_MAYBE_P(self->clustering_order));
    for (i = 0; i < count; ++i) {
      CassClusteringOrder order =
          cass_table_meta_clustering_key_order(self->meta, i);
      switch (order) {
        case CASS_CLUSTERING_ORDER_ASC:
          PHP8_ADD_NEXT_INDEX_STRING(PHP8_ZVAL_MAYBE_P(self->clustering_order), "asc");
          break;
        case CASS_CLUSTERING_ORDER_DESC:
          PHP8_ADD_NEXT_INDEX_STRING(PHP8_ZVAL_MAYBE_P(self->clustering_order), "desc");
          break;
        case CASS_CLUSTERING_ORDER_NONE:
          PHP8_ADD_NEXT_INDEX_STRING(PHP8_ZVAL_MAYBE_P(self->clustering_order), "none");
          break;
      }
    }
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->clustering_order), 1, 0);
}
...
PHP_METHOD(DefaultTable, index)
{
  ...
  if (PHP8_ZVAL_IS_UNDEF(index)) {
    return;
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(index), 0, 1);
}
...
PHP_METHOD(DefaultTable, indexes)
{
  ...
  while (cass_iterator_next(iterator)) {
    ...
    if (!PHP8_ZVAL_IS_UNDEF(zindex)) {
      php_driver_index *index = PHP_DRIVER_GET_INDEX(PHP8_ZVAL_MAYBE_P(zindex));

      if (PHP8_Z_TYPE_MAYBE_P(index->name) == IS_STRING) {
        PHP8_ADD_ASSOC_ZVAL_EX(return_value,
                                  PHP8_Z_STRVAL_MAYBE_P(index->name),
                                  PHP8_Z_STRLEN_MAYBE_P(index->name) + 1,
                                  PHP8_ZVAL_MAYBE_P(zindex));
      } else {
        add_next_index_zval(return_value, PHP8_ZVAL_MAYBE_P(zindex));
      }
    }
  }
...
PHP_METHOD(DefaultTable, materializedView)
{
  ...
  if (PHP8_ZVAL_IS_UNDEF(zview)) {
    return;
  }

  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(zview), 0, 1);
}
...
PHP_METHOD(DefaultTable, materializedViews)
{
  ...
  while (cass_iterator_next(iterator)) {
    ...
    if (!PHP8_ZVAL_IS_UNDEF(zview)) {
      view = PHP_DRIVER_GET_MATERIALIZED_VIEW(PHP8_ZVAL_MAYBE_P(zview));

      if (PHP8_Z_TYPE_MAYBE_P(view->name) == IS_STRING) {
        PHP8_ADD_ASSOC_ZVAL_EX(return_value,
                                  PHP8_Z_STRVAL_MAYBE_P(view->name),
                                  PHP8_Z_STRLEN_MAYBE_P(view->name) + 1,
                                  PHP8_ZVAL_MAYBE_P(zview));
      } else {
        add_next_index_zval(return_value, PHP8_ZVAL_MAYBE_P(zview));
      }
    }
  }
...
static void
php_driver_default_table_free(php8_zend_object_free *object)
{
  php_driver_table *self = PHP8_ZEND_OBJECT_GET(table, object);

  PHP8_ZVAL_MAYBE_DESTROY(self->name);
  PHP8_ZVAL_MAYBE_DESTROY(self->options);
  PHP8_ZVAL_MAYBE_DESTROY(self->partition_key);
  PHP8_ZVAL_MAYBE_DESTROY(self->primary_key);
  PHP8_ZVAL_MAYBE_DESTROY(self->clustering_key);
  PHP8_ZVAL_MAYBE_DESTROY(self->clustering_order);
...
{
  php_driver_table *self =
      PHP8_ZEND_OBJECT_ECALLOC(table, ce);

  PHP8_ZVAL_UNDEF(self->name);
  PHP8_ZVAL_UNDEF(self->options);
  PHP8_ZVAL_UNDEF(self->partition_key);
  PHP8_ZVAL_UNDEF(self->primary_key);
  PHP8_ZVAL_UNDEF(self->clustering_key);
  PHP8_ZVAL_UNDEF(self->clustering_order);

  self->meta   = NULL;
  self->schema = NULL;

  PHP8_ZEND_OBJECT_INIT_EX(table, default_table, self, ce);
}
