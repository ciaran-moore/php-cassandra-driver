...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_UUID TSRMLS_CC);
...
static void
php_driver_uuid_free(php8_zend_object_free *object)
{
  php_driver_uuid *self = PHP8_ZEND_OBJECT_GET(uuid, object);
...
php_driver_uuid_new(zend_class_entry *ce)
{
  ...
  PHP8_ZEND_OBJECT_INIT(uuid, self, ce);
}
