                         ...
                         const CassColumnMeta *meta)
{
  ...
  object_init_ex(PHP8_ZVAL_MAYBE_P(result), php_driver_default_column_ce);

  column = PHP_DRIVER_GET_COLUMN(PHP8_ZVAL_MAYBE_P(result));
  column->schema = php_driver_add_ref(schema);
  column->meta   = meta;

  cass_column_meta_name(meta, &name, &name_length);
  PHP8_ZVAL_MAYBE_MAKE(column->name);
  PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_P(column->name), name, name_length);
...
PHP_METHOD(DefaultColumn, type)
{
  ...
  if (PHP8_ZVAL_IS_UNDEF(self->type)) {
    RETURN_NULL();
  }
...
static void
php_driver_default_column_free(php8_zend_object_free *object)
{
  php_driver_column *self = PHP8_ZEND_OBJECT_GET(column, object);
...
{
  php_driver_column *self =
      PHP8_ZEND_OBJECT_ECALLOC(column, ce);
  ...
  PHP8_ZEND_OBJECT_INIT_EX(column, default_column, self, ce);
}
