...
static void
php_driver_future_close_free(php8_zend_object_free *object)
{
  php_driver_future_close *self =
      PHP8_ZEND_OBJECT_GET(future_close, object);
...
{
  php_driver_future_close *self =
      PHP8_ZEND_OBJECT_ECALLOC(future_close, ce);

  self->future = NULL;

  PHP8_ZEND_OBJECT_INIT(future_close, self, ce);
}
