...
static void
php_driver_timestamp_gen_monotonic_free(php8_zend_object_free *object)
{
  php_driver_timestamp_gen *self = PHP8_ZEND_OBJECT_GET(timestamp_gen, object);
...
{
  php_driver_timestamp_gen *self = PHP8_ZEND_OBJECT_ECALLOC(timestamp_gen, ce);

  self->gen = cass_timestamp_gen_monotonic_new();

  PHP8_ZEND_OBJECT_INIT_EX(timestamp_gen, timestamp_gen_monotonic, self, ce);
}
...
void php_driver_define_TimestampGeneratorMonotonic()
{
  ...
  zend_class_implements(php_driver_timestamp_gen_monotonic_ce TSRMLS_CC, 1, php_driver_timestamp_gen_ce);
