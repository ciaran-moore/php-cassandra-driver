...
PHP_METHOD(FutureSession, get)
{
  ...
  if (!PHP8_ZVAL_IS_UNDEF(self->default_session)) {
    RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->default_session), 1, 0);
  }
  ...
  if (rc != CASS_OK) {
    ...
    if (self->persist) {
      ...
      if (PHP8_ZEND_HASH_DEL(&EG(persistent_list), self->hash_key, self->hash_key_len + 1)) {
        self->future  = NULL;
      }
  ...
  PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(self->default_session), return_value);
}
...
static void
php_driver_future_session_free(php8_zend_object_free *object)
{
  php_driver_future_session *self =
      PHP8_ZEND_OBJECT_GET(future_session, object);
  ...
  PHP8_ZVAL_MAYBE_DESTROY(self->default_session);
...
{
  php_driver_future_session *self
      = PHP8_ZEND_OBJECT_ECALLOC(future_session, ce);
  ...
  PHP8_ZVAL_UNDEF(self->default_session);

  PHP8_ZEND_OBJECT_INIT(future_session, self, ce);
}
