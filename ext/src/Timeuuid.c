...
php_driver_timeuuid_init(INTERNAL_FUNCTION_PARAMETERS)
{
  ...
  if (ZEND_NUM_ARGS() == 0) {
    ...
    switch (Z_TYPE_P(param)) {
      case IS_LONG:
        if (Z_LVAL_P(param) < 0) {
          zend_throw_exception_ex(php_driver_invalid_argument_exception_ce, 0 TSRMLS_CC, "Timestamp must be a positive integer, %ld given", Z_LVAL_P(param));
          return;
        }
      ...
      case IS_STRING:
        if (cass_uuid_from_string(Z_STRVAL_P(param), &self->uuid) != CASS_OK) {
          zend_throw_exception_ex(php_driver_invalid_argument_exception_ce, 0 TSRMLS_CC, "Invalid UUID: '%s'", Z_STRVAL_P(param));
          return;
        }
...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_TIMEUUID TSRMLS_CC);
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(type), 1, 1);
}
/* }}} */
...
)
{
  ...
  php_driver_uuid *self = PHP8_ZEND_OBJECT_GET(uuid, object);
  HashTable      *props = zend_std_get_properties(object TSRMLS_CC);

  type = php_driver_type_scalar(CASS_VALUE_TYPE_TIMEUUID TSRMLS_CC);
  PHP8_ZEND_HASH_UPDATE(props, "type", sizeof("type"), PHP8_ZVAL_MAYBE_P(type), sizeof(zval));

  cass_uuid_string(self->uuid, string);

  PHP8_ZVAL_MAYBE_MAKE(uuid);
  PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(uuid), string);
  PHP8_ZEND_HASH_UPDATE(props, "uuid", sizeof("uuid"), PHP8_ZVAL_MAYBE_P(uuid), sizeof(zval));

  PHP8_ZVAL_MAYBE_MAKE(version);
  ZVAL_LONG(PHP8_ZVAL_MAYBE_P(version), (long) cass_uuid_version(self->uuid));
  PHP8_ZEND_HASH_UPDATE(props, "version", sizeof("version"), PHP8_ZVAL_MAYBE_P(version), sizeof(zval));
...
static void
php_driver_timeuuid_free(php8_zend_object_free *object)
{
  php_driver_uuid *self = PHP8_ZEND_OBJECT_GET(uuid, object);
...
{
  php_driver_uuid *self =
      PHP8_ZEND_OBJECT_ECALLOC(uuid, ce);

  PHP8_ZEND_OBJECT_INIT_EX(uuid, timeuuid, self, ce);
}
