...
{
  php8_zval type = php_driver_type_scalar(CASS_VALUE_TYPE_INET TSRMLS_CC);
...
)
{
  ...
  php_driver_inet *self = PHP8_ZEND_OBJECT_GET(inet, object);
  ...
  PHP8_ZVAL_MAYBE_MAKE(address);
  PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(address), string);
  efree(string);
  PHP8_ZEND_HASH_UPDATE(props, "address", sizeof("address"), PHP8_ZVAL_MAYBE_P(address), sizeof(zval));
...
static void
php_driver_inet_free(php8_zend_object_free *object)
{
  php_driver_inet *self = PHP8_ZEND_OBJECT_GET(inet, object);
...
{
  php_driver_inet *self =
      PHP8_ZEND_OBJECT_ECALLOC(inet, ce);

  PHP8_ZEND_OBJECT_INIT(inet, self, ce);
}
