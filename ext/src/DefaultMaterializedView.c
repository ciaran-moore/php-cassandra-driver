...
populate_partition_key(php_driver_materialized_view *view, zval *result)
{
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (column) {
      php8_zval zcolumn = php_driver_create_column(view->schema, column TSRMLS_CC);
                                       ...
                                       const CassMaterializedViewMeta *meta)
{
  ...
  object_init_ex(PHP8_ZVAL_MAYBE_P(result), php_driver_default_materialized_view_ce);

  view = PHP_DRIVER_GET_MATERIALIZED_VIEW(PHP8_ZVAL_MAYBE_P(result));
  view->schema = php_driver_add_ref(schema);
  view->meta   = meta;

  cass_materialized_view_meta_name(meta, &name, &name_length);
  PHP8_ZVAL_MAYBE_MAKE(view->name);
  PHP8_ZVAL_STRINGL(PHP8_ZVAL_MAYBE_P(view->name), name, name_length);
...
php_driver_materialized_view_get_option(php_driver_materialized_view *view,
                                           ...
                                           zval *result) {
  zval *zvalue;
  if (PHP8_ZVAL_IS_UNDEF(view->options)) {
    php_driver_default_materialized_view_build_options(view TSRMLS_CC);
  }

  if (!PHP8_ZEND_HASH_FIND(PHP8_Z_ARRVAL_MAYBE_P(view->options),
                         name, strlen(name) + 1,
                         zvalue)) {
    ZVAL_FALSE(result);
    return;
...
PHP_METHOD(DefaultMaterializedView, baseTable)
{
  ...
  self = PHP_DRIVER_GET_MATERIALIZED_VIEW(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->base_table)) {
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->base_table), 1, 0);
}
...
static void
php_driver_default_materialized_view_free(php8_zend_object_free *object)
{
  php_driver_materialized_view *self = PHP8_ZEND_OBJECT_GET(materialized_view, object);
  ...
  PHP8_ZVAL_MAYBE_DESTROY(self->base_table);
...
{
  php_driver_materialized_view *self =
      PHP8_ZEND_OBJECT_ECALLOC(materialized_view, ce);
  ...
  PHP8_ZVAL_UNDEF(self->base_table);

  self->meta   = NULL;
  self->schema = NULL;

  PHP8_ZEND_OBJECT_INIT_EX(materialized_view, default_materialized_view, self, ce);
}
...
void php_driver_define_DefaultMaterializedView()
{
  ...
  php_driver_default_materialized_view_ce = php8_zend_register_internal_class_ex(&ce, php_driver_materialized_view_ce);
