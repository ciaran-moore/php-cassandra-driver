...
php_driver_set_add(php_driver_set *set, zval *object)
{
  ...
  if (!php_driver_validate_object(object, PHP8_ZVAL_MAYBE_P(type->data.set.value_type) TSRMLS_CC)) {
    return 0;
  }
  ...
  if (entry == NULL) {
    ...
    PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(entry->value), object);
...
)
{
  ...
  php_driver_set *self = PHP8_ZEND_OBJECT_GET(set, object);
  ...
  php_driver_set_populate(self , PHP8_ZVAL_MAYBE_P(values) TSRMLS_CC);
...
php_driver_set_compare(zval *obj1, zval *obj2)
{
  ...
  type1 = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(set1->type));
  type2 = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(set2->type));
  ...
  HASH_ITER(hh, set1->entries, curr, temp) {
    php_driver_set_entry *entry;
    HASH_FIND_ZVAL(set2->entries, PHP8_ZVAL_MAYBE_P(curr->value), entry);
    if (entry == NULL) {
      return 1;
    }
  }
...
php_driver_set_hash_value(zval *obj)
{
  ...
  HASH_ITER(hh, self->entries, curr, temp) {
    hashv = php_driver_combine_hash(hashv, php_driver_value_hash(PHP8_ZVAL_MAYBE_P(curr->value) TSRMLS_CC));
  }
...
static void
php_driver_set_free(php8_zend_object_free *object)
{
  php_driver_set *self = PHP8_ZEND_OBJECT_GET(set, object);
  php_driver_set_entry *curr, *temp;
...
{
  php_driver_set *self =
      PHP8_ZEND_OBJECT_ECALLOC(set, ce);
  ...
  PHP8_ZEND_OBJECT_INIT(set, self, ce);
}
