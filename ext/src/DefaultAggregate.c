...
PHP_METHOD(DefaultAggregate, argumentTypes)
{
  ...
  self = PHP_DRIVER_GET_AGGREGATE(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->argument_types)) {
    size_t i, count = cass_aggregate_meta_argument_count(self->meta);
    PHP8_ZVAL_MAYBE_MAKE(self->argument_types);
    array_init(PHP8_ZVAL_MAYBE_P(self->argument_types));
    for (i = 0; i < count; ++i) {
      ...
      if (data_type) {
        ...
        if (!PHP8_ZVAL_IS_UNDEF(type)) {
          add_next_index_zval(PHP8_ZVAL_MAYBE_P(self->argument_types),
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->argument_types), 1, 0);
}
...
PHP_METHOD(DefaultAggregate, stateFunction)
{
  ...
  self = PHP_DRIVER_GET_AGGREGATE(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->state_function)) {
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->state_function), 1, 0);
}
...
PHP_METHOD(DefaultAggregate, finalFunction)
{
  ...
  self = PHP_DRIVER_GET_AGGREGATE(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->final_function)) {
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->final_function), 1, 0);
}
...
PHP_METHOD(DefaultAggregate, initialCondition)
{
  ...
  self = PHP_DRIVER_GET_AGGREGATE(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->initial_condition)) {
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->initial_condition), 1, 0);
}
...
PHP_METHOD(DefaultAggregate, stateType)
{
  ...
  self = PHP_DRIVER_GET_AGGREGATE(getThis());
  if (PHP8_ZVAL_IS_UNDEF(self->state_type)) {
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->state_type), 1, 0);
}
...
static void
php_driver_default_aggregate_free(php8_zend_object_free *object)
{
  php_driver_aggregate *self = PHP8_ZEND_OBJECT_GET(aggregate, object);

  PHP8_ZVAL_MAYBE_DESTROY(self->simple_name);
  PHP8_ZVAL_MAYBE_DESTROY(self->argument_types);
  PHP8_ZVAL_MAYBE_DESTROY(self->state_function);
  PHP8_ZVAL_MAYBE_DESTROY(self->final_function);
  PHP8_ZVAL_MAYBE_DESTROY(self->initial_condition);
  PHP8_ZVAL_MAYBE_DESTROY(self->state_type);
...
{
  php_driver_aggregate *self =
      PHP8_ZEND_OBJECT_ECALLOC(aggregate, ce);

  PHP8_ZVAL_UNDEF(self->simple_name);
  PHP8_ZVAL_UNDEF(self->argument_types);
  PHP8_ZVAL_UNDEF(self->state_function);
  PHP8_ZVAL_UNDEF(self->final_function);
  PHP8_ZVAL_UNDEF(self->initial_condition);
  PHP8_ZVAL_UNDEF(self->state_type);
  ...
  PHP8_ZEND_OBJECT_INIT_EX(aggregate, default_aggregate, self, ce);
}
