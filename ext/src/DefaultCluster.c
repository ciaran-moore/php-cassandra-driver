...
PHP_METHOD(DefaultCluster, connect)
{
  ...
  if (!PHP8_ZVAL_IS_UNDEF(session->default_timeout)) {
    PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(session->default_timeout),
...
static void
php_driver_default_cluster_free(php8_zend_object_free *object)
{
  php_driver_cluster *self = PHP8_ZEND_OBJECT_GET(cluster, object);
...
php_driver_default_cluster_new(zend_class_entry *ce)
{
  php_driver_cluster *self =
      PHP8_ZEND_OBJECT_ECALLOC(cluster, ce);
  ...
  PHP8_ZEND_OBJECT_INIT_EX(cluster, default_cluster, self, ce);
}
