...
PHP_METHOD(SSLOptionsBuilder, withTrustedCerts)
{
  ...
  for (i = 0; i < argc; i++) {
    zval *path = PHP8_ZVAL_ARG(args[i]);
    ...
    if (PHP8_ZVAL_IS_FALSE_P(&readable)) {
...
PHP_METHOD(SSLOptionsBuilder, withPrivateKey)
{
  ...
  php8_size private_key_len, passphrase_len;
  zval readable;
  php_driver_ssl_builder* builder = NULL;
...
static void
php_driver_ssl_builder_free(php8_zend_object_free *object)
{
  php_driver_ssl_builder *self = PHP8_ZEND_OBJECT_GET(ssl_builder, object);
...
php_driver_ssl_builder_new(zend_class_entry *ce)
{
  php_driver_ssl_builder *self =
      PHP8_ZEND_OBJECT_ECALLOC(ssl_builder, ce);
  ...
  PHP8_ZEND_OBJECT_INIT(ssl_builder, self, ce);
}
