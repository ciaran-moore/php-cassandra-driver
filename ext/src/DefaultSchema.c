...
PHP_METHOD(DefaultSchema, keyspaces)
{
  ...
  while (cass_iterator_next(iterator)) {
    ...
    PHP8_ZVAL_MAYBE_MAKE(zkeyspace);
    object_init_ex(PHP8_ZVAL_MAYBE_P(zkeyspace), php_driver_default_keyspace_ce);
    keyspace = PHP_DRIVER_GET_KEYSPACE(PHP8_ZVAL_MAYBE_P(zkeyspace));
    keyspace->schema = php_driver_add_ref(self->schema);
    keyspace->meta   = meta;
    PHP8_ADD_ASSOC_ZVAL_EX(return_value,
                              keyspace_name, keyspace_name_len + 1,
                              PHP8_ZVAL_MAYBE_P(zkeyspace));
  }
...
static void
php_driver_default_schema_free(php8_zend_object_free *object)
{
  php_driver_schema *self = PHP8_ZEND_OBJECT_GET(schema, object);
...
{
  php_driver_schema *self =
      PHP8_ZEND_OBJECT_ECALLOC(schema, ce);

  self->schema = NULL;

  PHP8_ZEND_OBJECT_INIT_EX(schema, default_schema, self, ce);
}
