...
php_driver_map_set(php_driver_map *map, zval *zkey, zval *zvalue)
{
  ...
  type = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(map->type));

  if (!php_driver_validate_object(zkey, PHP8_ZVAL_MAYBE_P(type->data.map.key_type) TSRMLS_CC)) {
    return 0;
  }

  if (!php_driver_validate_object(zvalue, PHP8_ZVAL_MAYBE_P(type->data.map.value_type) TSRMLS_CC)) {
    return 0;
  }
  ...
  if (entry == NULL) {
    ...
    PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(entry->key), zkey);
    PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(entry->value), zvalue);
    HASH_ADD_ZVAL(map->entries, key, entry);
  } else {
    php8_zval prev_value = entry->value;
    PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(entry->value), zvalue);
...
static int
php_driver_map_get(php_driver_map *map, zval *zkey, php8_zval *zvalue)
{
  php_driver_map_entry *entry;
  php_driver_type *type;
  int result = 0;
...
php_driver_map_populate_keys(const php_driver_map *map, zval *array)
{
  ...
  HASH_ITER(hh, map->entries, curr, temp) {
    if (add_next_index_zval(array, PHP8_ZVAL_MAYBE_P(curr->key)) != SUCCESS) {
      break;
    }
    Z_TRY_ADDREF_P(PHP8_ZVAL_MAYBE_P(curr->key));
  }
}
...
php_driver_map_populate_values(const php_driver_map *map, zval *array)
{
  php_driver_map_entry *curr, *temp;
  HASH_ITER(hh, map->entries, curr, temp) {
    if (add_next_index_zval(array, PHP8_ZVAL_MAYBE_P(curr->value)) != SUCCESS) {
      break;
    }
    Z_TRY_ADDREF_P(PHP8_ZVAL_MAYBE_P(curr->value));
  }
}
...
PHP_METHOD(Map, __construct)
{
  ...
  zval *key_type;
  zval *value_type;
  php8_zval scalar_key_type;
  php8_zval scalar_value_type;

  PHP8_ZVAL_UNDEF(scalar_key_type);
  PHP8_ZVAL_UNDEF(scalar_value_type);
  ...
  if (Z_TYPE_P(key_type) == IS_STRING) {
    ...
    key_type = PHP8_ZVAL_MAYBE_P(scalar_key_type);
  ...
  if (Z_TYPE_P(value_type) == IS_STRING) {
    ...
    value_type = PHP8_ZVAL_MAYBE_P(scalar_value_type);
    ...
    zval_ptr_dtor(PHP8_ZVAL_MAYBE_ADDR_OF(key_type));
...
PHP_METHOD(Map, get)
{
  ...
  if (php_driver_map_get(self, key, &value TSRMLS_CC))
    RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(value), 1, 0);
}
...
PHP_METHOD(Map, current)
{
  ...
  if (self->iter_curr != NULL)
    RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->iter_curr->value), 1, 0);
}
...
PHP_METHOD(Map, key)
{
  php_driver_map *self = PHP_DRIVER_GET_MAP(getThis());
  if (self->iter_curr != NULL)
    RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->iter_curr->key), 1, 0);
}
...
)
{
  ...
  php_driver_map *self = PHP8_ZEND_OBJECT_GET(map, object);
  ...
  PHP8_ZVAL_MAYBE_MAKE(keys);
  array_init(PHP8_ZVAL_MAYBE_P(keys));
  php_driver_map_populate_keys(self, PHP8_ZVAL_MAYBE_P(keys) TSRMLS_CC);
  PHP8_ZEND_HASH_SORT(Z_ARRVAL_P(PHP8_ZVAL_MAYBE_P(keys)), php_driver_data_compare, 1);
  PHP8_ZEND_HASH_UPDATE(props, "keys", sizeof("keys"), PHP8_ZVAL_MAYBE_P(keys), sizeof(zval *));

  PHP8_ZVAL_MAYBE_MAKE(values);
  array_init(PHP8_ZVAL_MAYBE_P(values));
  php_driver_map_populate_values(self, PHP8_ZVAL_MAYBE_P(values) TSRMLS_CC);
  PHP8_ZEND_HASH_SORT(Z_ARRVAL_P(PHP8_ZVAL_MAYBE_P(values)), php_driver_data_compare, 1);
...
php_driver_map_compare(zval *obj1, zval *obj2)
{
  ...
  type1 = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(map1->type));
  type2 = PHP_DRIVER_GET_TYPE(PHP8_ZVAL_MAYBE_P(map2->type));
  ...
  HASH_ITER(hh, map1->entries, curr, temp) {
    ...
    HASH_FIND_ZVAL(map2->entries, PHP8_ZVAL_MAYBE_P(curr->key), entry);
    if (entry == NULL) {
      return 1;
    }
    result = php_driver_value_compare(PHP8_ZVAL_MAYBE_P(curr->value),
                                      PHP8_ZVAL_MAYBE_P(entry->value) TSRMLS_CC);
    if (result != 0) return result;
  }
...
php_driver_map_hash_value(zval *obj)
{
  ...
  HASH_ITER(hh, self->entries, curr, temp) {
    hashv = php_driver_combine_hash(hashv,
                                       php_driver_value_hash(PHP8_ZVAL_MAYBE_P(curr->key) TSRMLS_CC));
    hashv = php_driver_combine_hash(hashv,
                                       php_driver_value_hash(PHP8_ZVAL_MAYBE_P(curr->value) TSRMLS_CC));
  }
...
static void
php_driver_map_free(php8_zend_object_free *object)
{
  php_driver_map *self = PHP8_ZEND_OBJECT_GET(map, object);
  php_driver_map_entry *curr, *temp;
