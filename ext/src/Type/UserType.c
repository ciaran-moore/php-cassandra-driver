                                     ...
                                     zval *zsub_type)
{
  ...
  PHP8_ZEND_HASH_ADD(&type->data.udt.types,
                        name, name_length + 1,
...
PHP_METHOD(TypeUserType, withName)
{
  ...
  PHP8_ZEND_HASH_ZVAL_COPY(&user_type->data.udt.types, &self->data.udt.types);
}
...
PHP_METHOD(TypeUserType, name)
{
  ...
  PHP8_RETVAL_STRING(self->data.udt.type_name);
}
...
PHP_METHOD(TypeUserType, keyspace)
{
  ...
  PHP8_RETVAL_STRING(self->data.udt.keyspace);
}
...
PHP_METHOD(TypeUserType, types)
{
  ...
  PHP8_ZEND_HASH_ZVAL_COPY(Z_ARRVAL_P(return_value), &self->data.udt.types);
}
...
PHP_METHOD(TypeUserType, create)
{
  ...
  PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(user_type_value->type), getThis());
  ...
  if (argc > 0) {
    ...
    for (i = 0; i < argc; i += 2) {
      zval *name = PHP8_ZVAL_ARG(args[i]);
      zval *value = PHP8_ZVAL_ARG(args[i + 1]);
      php8_zval *sub_type;
      ...
      if (!PHP8_ZEND_HASH_FIND(&self->data.udt.types,
...
)
{
  ...
  PHP8_ZEND_HASH_ZVAL_COPY(PHP8_Z_ARRVAL_MAYBE_P(types), &self->data.udt.types);
...
static void
php_driver_type_user_type_free(php8_zend_object_free *object)
{
  php_driver_type *self = PHP8_ZEND_OBJECT_GET(type, object);
...
php_driver_type_user_type_new(zend_class_entry *ce)
{
  ...
  PHP8_ZEND_OBJECT_INIT_EX(type, type_user_type, self, ce);
}
...
void php_driver_define_TypeUserType()
{
  ...
  php_driver_type_user_type_ce = php8_zend_register_internal_class_ex(&ce, php_driver_type_ce);
