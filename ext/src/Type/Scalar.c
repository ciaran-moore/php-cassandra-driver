...
)
{
  ...
  PHP8_ZVAL_STRING(PHP8_ZVAL_MAYBE_P(name),
...
static void
php_driver_type_scalar_free(php8_zend_object_free *object)
{
  php_driver_type *self = PHP8_ZEND_OBJECT_GET(type, object);
...
php_driver_type_scalar_new(zend_class_entry *ce)
{
  ...
  PHP8_ZEND_OBJECT_INIT_EX(type, type_scalar, self, ce);
}
...
void php_driver_define_TypeScalar()
{
  ...
  php_driver_type_scalar_ce = php8_zend_register_internal_class_ex(&ce, php_driver_type_ce);
