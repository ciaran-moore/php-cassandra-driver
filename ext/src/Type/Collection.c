...
PHP_METHOD(TypeCollection, valueType)
{
  ...
  RETURN_ZVAL(PHP8_ZVAL_MAYBE_P(self->data.collection.value_type), 1, 0);
}
...
PHP_METHOD(TypeCollection, create)
{
  ...
  PHP8_ZVAL_COPY(PHP8_ZVAL_MAYBE_P(collection->type), getThis());

  if (argc > 0) {
    for (i = 0; i < argc; i++) {
      if (!php_driver_validate_object(PHP8_ZVAL_ARG(args[i]),
                                      PHP8_ZVAL_MAYBE_P(self->data.collection.value_type) TSRMLS_CC)) {
        PHP8_MAYBE_EFREE(args);
        return;
      }

      php_driver_collection_add(collection, PHP8_ZVAL_ARG(args[i]) TSRMLS_CC);
    }
...
)
{
  ...
  PHP8_ZEND_HASH_UPDATE(props,
                           ...
                           PHP8_ZVAL_MAYBE_P(self->data.collection.value_type), sizeof(zval));
  Z_ADDREF_P(PHP8_ZVAL_MAYBE_P(self->data.collection.value_type));
...
static void
php_driver_type_collection_free(php8_zend_object_free *object)
{
  php_driver_type *self = PHP8_ZEND_OBJECT_GET(type, object);

  if (self->data_type) cass_data_type_free(self->data_type);
  PHP8_ZVAL_MAYBE_DESTROY(self->data.collection.value_type);
...
php_driver_type_collection_new(zend_class_entry *ce)
{
  ...
  PHP8_ZVAL_UNDEF(self->data.collection.value_type);

  PHP8_ZEND_OBJECT_INIT_EX(type, type_collection, self, ce);
}
...
void php_driver_define_TypeCollection()
{
  ...
  php_driver_type_collection_ce = php8_zend_register_internal_class_ex(&ce, php_driver_type_ce);
